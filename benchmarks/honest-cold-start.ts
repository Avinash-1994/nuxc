...
async function testDevServerColdStart(): Promise<BenchmarkResult> {
    ...
    const cacheDir = path.join(process.cwd(), 'nuce-web-app', 'node_modules', '.nuce');
    ...
    return new Promise((resolve) => {
        ...
        // We use npm run dev which calls 'nuce dev'
        const appDir = path.join(process.cwd(), 'nuce-web-app');
...
async function runHonestBenchmarks() {
    ...
    if (coldStartResult) {
        ...
        console.log(`  Nuce:     ${coldStartResult.duration}ms`);
        ...
        if (coldStartResult.duration < 425) {
            console.log(`\n  ✅ Nuce is ${Math.round((425 - coldStartResult.duration) / 425 * 100)}% faster than Vite!`);
        } else {
            console.log(`\n  ⚠️  Nuce is ${Math.round((coldStartResult.duration - 425) / 425 * 100)}% slower than Vite`);
        }

        if (coldStartResult.duration < 200) {
            console.log(`  🏆 Nuce BEATS esbuild target!`);
        } else {
            console.log(`  📊 Nuce is ${Math.round((coldStartResult.duration - 200) / 200 * 100)}% slower than esbuild target`);
        }
    }
