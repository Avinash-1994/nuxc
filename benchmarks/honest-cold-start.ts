...
async function testDevServerColdStart(): Promise<BenchmarkResult> {
    ...
    const cacheDir = path.join(process.cwd(), 'nuxco-web-app', 'node_modules', '.nuxco');
    ...
    return new Promise((resolve) => {
        ...
        // We use npm run dev which calls 'nuxco dev'
        const appDir = path.join(process.cwd(), 'nuxco-web-app');
...
async function runHonestBenchmarks() {
    ...
    if (coldStartResult) {
        ...
        console.log(`  Nuxco:     ${coldStartResult.duration}ms`);
        ...
        if (coldStartResult.duration < 425) {
            console.log(`\n  ✅ Nuxco is ${Math.round((425 - coldStartResult.duration) / 425 * 100)}% faster than Vite!`);
        } else {
            console.log(`\n  ⚠️  Nuxco is ${Math.round((coldStartResult.duration - 425) / 425 * 100)}% slower than Vite`);
        }

        if (coldStartResult.duration < 200) {
            console.log(`  🏆 Nuxco BEATS esbuild target!`);
        } else {
            console.log(`  📊 Nuxco is ${Math.round((coldStartResult.duration - 200) / 200 * 100)}% slower than esbuild target`);
        }
    }
