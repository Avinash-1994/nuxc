...
{"name":"nuxc-sveltekit-template","version":"0.0.1","private":true,"type":"module","scripts":{"dev":"nuxc dev","build":"nuxc build","preview":"nuxc preview"},"nuxc":{"template":true,"framework":"sveltekit","description":"Nuxc Blog — full-stack blog with CMS"},"dependencies":{"@sveltejs/kit":"2.5.18","svelte":"4.2.18","marked":"12.0.0"},"devDependencies":{"nuxc":"file:../..","typescript":"5.4.5"}}
EOF
cat > "$T/sveltekit/nuxc.config.ts" <<'EOF'
...
<svelte:head><title>Nuxc Blog</title></svelte:head>
<main style="max-width:800px;margin:0 auto;padding:32px;font-family:system-ui;background:#0f172a;min-height:100vh;color:#f1f5f9">
  <h1 style="font-size:36px;margin-bottom:8px">⚡ Nuxc Blog</h1>
  <p style="color:#94a3b8;margin-bottom:32px">Engineering insights from the Nuxc team</p>
...
export const load = async () => ({
  posts: [
    { slug:'nuxc-1-0-release', title:'Nuxc 1.0: Production-Ready Build Tool', category:'Engineering', date:'2026-05-14', excerpt:'After 303 tests and 6 months of development, Nuxc 1.0 is ready. Here\'s what changed.', tags:['release','engineering'] },
    { slug:'sveltekit-ssr-deep-dive', title:'SvelteKit SSR with Nuxc: Zero Config', category:'Tutorial', date:'2026-05-10', excerpt:'How Nuxc auto-detects SvelteKit and configures SSR with no config needed.', tags:['sveltekit','ssr','tutorial'] },
    ...
    { slug:'module-federation-2026', title:'Module Federation in 2026', category:'Architecture', date:'2026-05-01', excerpt:'Nuxc brings native MFE support across React, Vue, and Angular with zero configuration.', tags:['mfe','architecture'] },
  ]
});
EOF
...
{"name":"nuxc-remix-template","version":"0.0.1","private":true,"type":"module","scripts":{"dev":"nuxc dev","build":"nuxc build","preview":"nuxc preview"},"nuxc":{"template":true,"framework":"remix","description":"Nuxc Jobs — job board with applications"},"dependencies":{"@remix-run/react":"2.9.2","@remix-run/node":"2.9.2","react":"18.3.1","react-dom":"18.3.1"},"devDependencies":{"nuxc":"file:../..","typescript":"5.4.5"}}
EOF
cat > "$T/remix/nuxc.config.ts" <<'EOF'
import { defineConfig } from 'nuxc';
...
export default function Login() {
  return (
    <div style={{padding:32,maxWidth:400,fontFamily:'system-ui',background:'#0f172a',minHeight:'100vh',color:'#f1f5f9'}}>
      <h1>Sign In</h1>
      <Form method="post" style={{display:'flex',flexDirection:'column',gap:16,marginTop:24}}>
        <input type="email" name="email" placeholder="Email" defaultValue="dev@nuxc.dev" required style={{padding:12,borderRadius:8,border:'1px solid #334155',background:'#1e293b',color:'#fff'}}/>
...
{"name":"nuxc-astro-template","version":"0.0.1","private":true,"type":"module","scripts":{"dev":"nuxc dev","build":"nuxc build","preview":"nuxc preview"},"nuxc":{"template":true,"framework":"astro","description":"Nuxc Docs — documentation site"},"dependencies":{"astro":"4.10.2"},"devDependencies":{"nuxc":"file:../..","typescript":"5.4.5"}}
EOF
cat > "$T/astro/nuxc.config.ts" <<'EOF'
import { defineConfig } from 'nuxc';
...
<body>
  <nav>
    <div style="font-weight:700;font-size:18px;margin-bottom:24px">⚡ Nuxc Docs</div>
...
---
---
<html lang="en"><head><meta charset="UTF-8"/><title>Installation — Nuxc Docs</title><style>body{background:#0f172a;color:#f1f5f9;font-family:system-ui;padding:48px;max-width:800px;margin:0 auto}pre{background:#1e293b;padding:20px;border-radius:10px;overflow-x:auto}code{color:#818cf8;font-family:monospace}</style></head>
<body>
  ...
  <pre><code>npm install -g nuxc</code></pre>
  <h2 style="margin-top:24px">Per-project install</h2>
  <pre><code>npm install -D nuxc</code></pre>
  <h2 style="margin-top:24px">Verify</h2>
  <pre><code>nuxc --version
# ⚡ Nuxc v1.0.10</code></pre>
</body></html>
EOF
...
{"name":"nuxc-mfe-template","version":"0.0.1","private":true,"type":"module","scripts":{"dev:all":"concurrently \"npm run dev:remote-vue\" \"npm run dev:remote-react\" \"npm run dev:host\"","dev:host":"cd host && nuxc dev","dev:remote-vue":"cd remote-vue && nuxc dev","dev:remote-react":"cd remote-react && nuxc dev","build:all":"npm run build --prefix remote-vue && npm run build --prefix remote-react && npm run build --prefix host"},"nuxc":{"template":true,"framework":"mfe","description":"Nuxc MFE — Module Federation host + 2 remotes"},"devDependencies":{"concurrently":"8.2.2"}}
EOF
cat > "$T/mfe/host/package.json" <<'EOF'
{"name":"nuxc-mfe-host","version":"0.0.1","private":true,"type":"module","scripts":{"dev":"nuxc dev","build":"nuxc build"},"dependencies":{"react":"18.3.1","react-dom":"18.3.1"},"devDependencies":{"nuxc":"file:../../.."}}
EOF
cat > "$T/mfe/host/nuxc.config.ts" <<'EOF'
import { defineConfig } from 'nuxc';
...
function App() {
  return (
    <div style={{minHeight:'100vh',background:'#0f172a',color:'#f1f5f9',fontFamily:'system-ui',padding:32}}>
      <h1 style={{marginBottom:32}}>⚡ Nuxc MFE Host</h1>
...
{"name":"nuxc-mfe-remote-vue","version":"0.0.1","private":true,"type":"module","scripts":{"dev":"nuxc dev","build":"nuxc build"},"dependencies":{"vue":"3.4.27"},"devDependencies":{"nuxc":"file:../../.."}}
EOF
cat > "$T/mfe/remote-vue/nuxc.config.ts" <<'EOF'
import { defineConfig } from 'nuxc';
export default defineConfig({
  framework: 'vue',
  dev: { port: 5174 },
...
{"name":"nuxc-mfe-remote-react","version":"0.0.1","private":true,"type":"module","scripts":{"dev":"nuxc dev","build":"nuxc build"},"dependencies":{"react":"18.3.1","react-dom":"18.3.1"},"devDependencies":{"nuxc":"file:../../.."}}
EOF
cat > "$T/mfe/remote-react/nuxc.config.ts" <<'EOF'
import { defineConfig } from 'nuxc';
export default defineConfig({
  framework: 'react',
  dev: { port: 5175 },
