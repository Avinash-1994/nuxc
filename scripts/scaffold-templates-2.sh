...
{"name":"nuxc-svelte-template","version":"0.0.1","private":true,"type":"module","scripts":{"dev":"nuxc dev","build":"nuxc build","preview":"nuxc preview"},"nuxc":{"template":true,"framework":"svelte","description":"Nuxc Notes — markdown note-taking app"},"dependencies":{"svelte":"4.2.18","marked":"12.0.0"},"devDependencies":{"nuxc":"file:../..","typescript":"5.4.5"}}
EOF
cat > "$T/svelte/nuxc.config.ts" <<'EOF'
import { defineConfig } from 'nuxc';
export default defineConfig({ framework: 'svelte' });
EOF
cat > "$T/svelte/index.html" <<'EOF'
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>Nuxc Notes</title></head><body><div id="app"></div><script type="module" src="/src/main.ts"></script></body></html>
EOF
...
{"name":"nuxc-solid-template","version":"0.0.1","private":true,"type":"module","scripts":{"dev":"nuxc dev","build":"nuxc build","preview":"nuxc preview"},"nuxc":{"template":true,"framework":"solid","description":"Nuxc Analytics — website analytics dashboard"},"dependencies":{"solid-js":"1.8.17"},"devDependencies":{"nuxc":"file:../..","typescript":"5.4.5"}}
EOF
cat > "$T/solid/nuxc.config.ts" <<'EOF'
import { defineConfig } from 'nuxc';
export default defineConfig({ framework: 'solid' });
EOF
cat > "$T/solid/index.html" <<'EOF'
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>Nuxc Analytics</title></head><body><div id="root"></div><script type="module" src="/src/main.tsx"></script></body></html>
EOF
  ...
  return (
    <div style="min-height:100vh;background:#0f172a;color:#f1f5f9;font-family:system-ui">
      <nav style="background:#1e293b;padding:0 24px;display:flex;gap:24px;align-items:center;height:56px">
        <span style="font-weight:700;font-size:18px;margin-right:16px">📊 Nuxc Analytics</span>
...
{"name":"nuxc-preact-template","version":"0.0.1","private":true,"type":"module","scripts":{"dev":"nuxc dev","build":"nuxc build","preview":"nuxc preview"},"nuxc":{"template":true,"framework":"preact","description":"Nuxc Weather — real weather dashboard"},"dependencies":{"preact":"10.22.0"},"devDependencies":{"nuxc":"file:../..","typescript":"5.4.5"}}
EOF
cat > "$T/preact/nuxc.config.ts" <<'EOF'
import { defineConfig } from 'nuxc';
export default defineConfig({ framework: 'preact' });
EOF
cat > "$T/preact/index.html" <<'EOF'
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>Nuxc Weather</title></head><body><div id="app"></div><script type="module" src="/src/main.tsx"></script></body></html>
EOF
...
{"name":"nuxc-vanilla-template","version":"0.0.1","private":true,"type":"module","scripts":{"dev":"nuxc dev","build":"nuxc build","preview":"nuxc preview"},"nuxc":{"template":true,"framework":"vanilla","description":"Nuxc Kanban — drag-and-drop board"},"devDependencies":{"nuxc":"file:../..","typescript":"5.4.5"}}
EOF
cat > "$T/vanilla/nuxc.config.ts" <<'EOF'
import { defineConfig } from 'nuxc';
export default defineConfig({ framework: 'vanilla', entry: 'src/main.ts' });
EOF
cat > "$T/vanilla/index.html" <<'EOF'
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>Nuxc Kanban</title><style>*{box-sizing:border-box;margin:0}body{background:#0f172a;color:#f1f5f9;font-family:system-ui;min-height:100vh}</style></head><body><div id="app"></div><script type="module" src="/src/main.ts"></script></body></html>
EOF
