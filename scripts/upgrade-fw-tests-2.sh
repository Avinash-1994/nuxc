...
# ── SVELTEKIT — sparx-blog ────────────────────────────────────────
mkdir -p "$FT/sparx-blog/src/routes/blog"
cat > "$FT/sparx-blog/package.json" <<'EOF'
{"name":"sparx-blog","version":"0.0.1","private":true,"type":"module",
"scripts":{"dev":"node ../../dist/cli.js dev","build":"node ../../dist/cli.js build"},
"dependencies":{"@sveltejs/kit":"2.5.18","svelte":"4.2.18"},
"devDependencies":{"typescript":"5.4.5"}}
EOF
cat > "$FT/sparx-blog/sparx.config.ts" <<'EOF'
const defineConfig = (c: any) => c;
export default defineConfig({ framework: 'sveltekit' });
EOF
cat > "$FT/sparx-blog/index.html" <<'EOF'
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>Sparx Blog</title></head><body><div id="app"></div><script type="module" src="/src/routes/+page.svelte"></script></body></html>
EOF
cat > "$FT/sparx-blog/src/routes/+page.svelte" <<'EOF'
<script lang="ts">
  const posts = [
    {slug:'sparx-1-0',title:'Sparx 1.0: Production-Ready Build Tool',date:'2026-05-14',category:'Engineering',excerpt:'303 tests, 19 framework adapters, security gate — Sparx 1.0 is ready.'},
    {slug:'sveltekit-ssr',title:'SvelteKit SSR with Sparx: Zero Config',date:'2026-05-10',category:'Tutorial',excerpt:'How Sparx auto-detects SvelteKit and configures SSR with no config.'},
    ...
    {slug:'hmr-performance',title:'How Sparx HMR Hits 12ms p50',date:'2026-04-28',category:'Performance',excerpt:'The uWS + WebSocket pipeline that makes 12ms HMR possible.'},
  ];
</script>
<div style="min-height:100vh;background:#0f172a;color:#f1f5f9;font-family:system-ui">
  <header style="background:#1e293b;padding:24px;border-bottom:1px solid #334155">
    <h1 style="margin:0;font-size:28px">⚡ Sparx Blog</h1>
    <p style="color:#94a3b8;margin:8px 0 0">Engineering insights from the Sparx team</p>
  </header>
...
# ── REMIX — sparx-jobs ────────────────────────────────────────────
mkdir -p "$FT/sparx-jobs/app/routes"
cat > "$FT/sparx-jobs/package.json" <<'EOF'
{"name":"sparx-jobs","version":"0.0.1","private":true,"type":"module",
"scripts":{"dev":"node ../../dist/cli.js dev","build":"node ../../dist/cli.js build"},
"dependencies":{"react":"18.3.1","react-dom":"18.3.1"},
"devDependencies":{"typescript":"5.4.5"}}
EOF
cat > "$FT/sparx-jobs/sparx.config.ts" <<'EOF'
const defineConfig = (c: any) => c;
export default defineConfig({ framework: 'remix' });
EOF
cat > "$FT/sparx-jobs/index.html" <<'EOF'
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>Sparx Jobs</title></head><body><div id="root"></div><script type="module" src="/app/routes/index.tsx"></script></body></html>
EOF
cat > "$FT/sparx-jobs/app/routes/index.tsx" <<'EOF'
...
const JOBS = [
  ...
  {id:2,title:'Staff Engineer — Build Tools',company:'Sparx Inc.',location:'Remote / SF',salary:'$200k–$250k',stack:['Rust','TypeScript','Node']},
...
# ── VANILLA KANBAN — sparx-kanban ────────────────────────────────
mkdir -p "$FT/sparx-kanban/src"
cat > "$FT/sparx-kanban/package.json" <<'EOF'
{"name":"sparx-kanban","version":"0.0.1","private":true,"type":"module",
"scripts":{"dev":"node ../../dist/cli.js dev","build":"node ../../dist/cli.js build"},
"devDependencies":{"typescript":"5.4.5"}}
EOF
cat > "$FT/sparx-kanban/sparx.config.ts" <<'EOF'
const defineConfig = (c: any) => c;
export default defineConfig({ framework: 'vanilla', entry: 'src/main.ts' });
EOF
cat > "$FT/sparx-kanban/index.html" <<'EOF'
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Sparx Kanban</title><style>*{box-sizing:border-box;margin:0}body{background:#0f172a;color:#f1f5f9;font-family:system-ui}</style></head><body><div id="app"></div><script type="module" src="/src/main.ts"></script></body></html>
EOF
cat > "$FT/sparx-kanban/src/main.ts" <<'EOF'
...
function render(){
  ...
  app.innerHTML=`<div style="padding:24px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px">
      <h1 style="font-size:24px">🗂️ Sparx Kanban</h1>
...
{"name":"sparx-mfe-host","version":"0.0.1","private":true,"type":"module",
"scripts":{"dev":"node ../../../dist/cli.js dev","build":"node ../../../dist/cli.js build"},
"dependencies":{"react":"18.3.1","react-dom":"18.3.1"},
"devDependencies":{"typescript":"5.4.5"}}
EOF
cat > "$FT/mfe-demo/host/sparx.config.ts" <<'EOF'
const defineConfig=(c:any)=>c;
...
function App() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0f172a',color:'#f1f5f9',fontFamily:'system-ui',padding:32}}>
      <h1 style={{marginBottom:24}}>⚡ Sparx MFE Host (React)</h1>
...
{"name":"sparx-mfe-remote","version":"0.0.1","private":true,"type":"module",
"scripts":{"dev":"node ../../../dist/cli.js dev","build":"node ../../../dist/cli.js build"},
"dependencies":{"react":"18.3.1","react-dom":"18.3.1"},
"devDependencies":{"typescript":"5.4.5"}}
EOF
cat > "$FT/mfe-demo/remote/sparx.config.ts" <<'EOF'
const defineConfig=(c:any)=>c;
export default defineConfig({
  framework:'react',
  dev:{ port:5175 },
...
echo "✅ sparx-blog, sparx-jobs, sparx-kanban, mfe-demo created"
