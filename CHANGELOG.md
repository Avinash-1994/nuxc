...
- **Developer Experience (Hero Tools)**
  - `zeptr verify`: Comprehensive project health diagnostics.
  - `zeptr analyze`: Build profiling and bundle composition visualization.
  - `zeptr report`: AI-augmented build session narration.
...
- **TypeScript Config Support**
  - Load `zeptr.build.ts` configuration
...
**TypeScript Config:**
You can now use `zeptr.build.ts` instead of JSON:
```typescript
export default {
  entry: ['src/main.tsx'],
  outDir: 'dist'
