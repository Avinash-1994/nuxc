...
- **CLI & Config**: Loads environment, parses `nuce.config.js`, and manages the build/dev lifecycle.
...
Nuce ensures 100% deterministic builds by calculating a **Global Fingerprint** before every execution.

- **Storage**: Highly efficient SQLite database (`.nuce_cache/build.db`).
- **Fingerprint Scope**:
