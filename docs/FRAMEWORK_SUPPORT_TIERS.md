...
Nuxco supports **ALL frameworks via adapters**, but we believe in **honest maturity levels** rather than false promises.
...
✅ **nuxco verify Support** - Full validation and health checks  
...
✅ **CSS Correctness** - CSS as first-class graph node  
✅ **Graph-Based Rebuilds** - Only rebuild affected modules  
✅ **nuxco verify Support** - Basic validation  
...
```typescript
// packages/@nuxco/framework-yourframework/src/index.ts
import { FrameworkAdapter } from 'nuxco';
...
**Remember**: Nuxco is about **correctness first, speed second**. Every tier gets deterministic builds and CSS correctness. The difference is in the DX polish.
