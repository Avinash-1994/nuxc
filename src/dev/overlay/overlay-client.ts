...
function getOverlay() {
    if (!overlayInstance || !overlayInstance.isConnected) {
        overlayInstance = document.createElement('nuxco-error-overlay');
...
(window as any).__NUXCO_OVERLAY__ = {
...
export function activateOverlay() {
    ...
    console.log('[Nuxco] Error Overlay Active');
}
