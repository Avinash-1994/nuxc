...
export async function startDevServer(cfg: BuildConfig) {
    ...
    const server = http.createServer(async (req, res) => {
        if (features && (server as any).__nuce_handler) {
            return (server as any).__nuce_handler(req, res);
        }
        ...
        if (!expectsHtml) {
            await initPromise;
            if ((server as any).__nuce_handler) {
                return (server as any).__nuce_handler(req, res);
            }
        }
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        server.listen(port, host, async () => {
            ...
            // Futuristic Nuce Branding (Engineering First)
            console.log(`\x1b[36m   ⚡ NUCE \x1b[90mv${pkgVersion}\x1b[0m`);
            ...
            try {
                ...
                const dbPath = cfg.cacheDir ?? pathModule.join(cfg.root || process.cwd(), '.nuce/cache/cache.db');
