...
try {
  const candidates = [
    path.resolve(__dirname, '../../nuxco_native.node'), // From src/dev/devServer.ts
    path.resolve(__dirname, '../nuxco_native.node'),    // From dist/dev/devServer.js
    path.resolve(__dirname, './nuxco_native.node'),     // From dist/
    path.resolve(process.cwd(), 'nuxco_native.node'),   // Root fallback
      ...
      if (node && node.type === 'Literal' && typeof node.value === 'string') {
        ...
        if (singletonRedirects && singletonRedirects.size > 0) {
          ...
          if (singletonRedirects.has(pkgRoot)) {
            ...
            const singletonUrl = `${hostBase}/@nuxco-deps/${safeName}.js?v=${Date.now()}`;
        ...
        if (preBundledDeps && preBundledDeps.has(specifier)) {
          ...
          if (preBundledDeps.has(pkgName)) {
            ...
            if (specifier.endsWith('.js') && !preBundledDeps.has(specifier)) {
              ...
              replacement = `/@nuxco-deps/${safeName}.js?v=${Date.now()}`;
            }
          }
        }
...
export async function startDevServer(cliCfg: BuildConfig, existingServer?: any) {
    ...
    pluginManager.register({
      ...
      transform(code, id) {
        ...
        return code.replace(regex, (_, remote, modulePath) => {
          return `globalThis.__nuxco_import__("${remote}/${modulePath}")`;
        });
      }
    });
  }
  ...
  // Phase 1.10: resolve cache dir from config. Default: '<root>/.nuxco/cache'.
  const resolvedCacheDir = cfg.cacheDir
    ? path.join(cfg.root, cfg.cacheDir)
    : path.join(cfg.root, '.nuxco', 'cache');
  ...
  try {
    ...
    if (prebundleConfig.enabled !== false) {
      ...
      // Nuxco understands the SSR boundary better than any other build tool.
  ...
  const requestHandler = async (req: http.IncomingMessage, res: http.ServerResponse) => {
    ...
    // proxy pre-bundled dep requests (/@nuxco-deps/react, etc.) to the shell
    // so there is exactly one React instance across all federation boundaries.
    if (cfg.federation?.exposes && cfg.federation.shared && req.url?.startsWith('/@nuxco-deps/')) {
      ...
      const rawDepPath = req.url.slice('/@nuxco-deps/'.length).split('?')[0];
      // Convert nuxco safe-name back to pkg name: "react-dom_client.js" → check both "react-dom" and "react"
      ...
      if (matchedSingleton) {
        ...
        const proxyUrl = `${singletonHost}/@nuxco-deps/${rawDepPath}${req.url.includes('?') ? '?' + req.url.split('?')[1] : ''}`;
        try {
    ...
    if (activeAdapter && typeof activeAdapter.getDevHandler === 'function') {
      ...
      (req as any).__nuxcoRoot = cfg.root;
    ...
    if (!anomalyDetector.scanRequest({ url: req.url || '', headers: req.headers, method: req.method || 'GET' })) {
      ...
      res.end('Request Blocked by Nuxco Security Shield');
      return;
    }

    if (req.url === '/__nuxco/security') {
    ...
    if (req.url === '/__nuxco/registry') {
      const snapshot = (globalThis as any).__nuxco_registry__?.getRegistry?.() ?? { scopes: {} };
      res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (cfg.preset === 'ssr' && (url === '/' || (!hasExtension && !isInternal && acceptsHtml))) {
      try {
        ...
        const ssrRunner = await import('../../packages/nuxco-ssr/src/runner.js')
          .catch(() => null as any);
        ...
        if (ssrRunner?.renderToString) {
          ...
          if (entryPath) {
            ...
            const finalHtml = shell
              .replace('<!--head-->', head + `<script>window.__NUXCO_STATE__=${state}</script>`)
              .replace('<!--app-->', html)
        ...
        log.warn(`[nuxco:ssr] renderToString failed for ${url}: ${e.message} — falling through`);
      }
    }
          ...
          if (cfg.federation.shared) {
            ...
            for (const [pkg, sharedCfg] of Object.entries(cfg.federation.shared)) {
              ...
              if (isSingleton) {
                ...
                singletonEntries.push(`    "${pkg}": "${hostOrigin}/@nuxco-deps/${pkg}"`);
    ...
    <script>
      ...
      window.$RefreshSig$ = () => (type) => type;
        ...
        if (isReact) {
          if (process.env.DEBUG) {
            log.info('[Nuxco] Injecting React Refresh Preamble', { category: 'server' });
          }
          preamble += `
    <script type="module">
      ...
      window.__vite_plugin_react_preamble_installed__ = true;
        ...
        let registryScript = '';
        try {
          const { generateRegistryInitTag } = await import('../../packages/nuxco-module-registry/src/browser-runtime.js')
            // @ts-ignore
            .catch(() => import('../../../packages/nuxco-module-registry/src/browser-runtime.js')
    ...
    if (url.startsWith('/@nuxco-deps/')) {
      // Strip query parameters
      const cleanUrl = url.split('?')[0];
      const depFile = cleanUrl.replace('/@nuxco-deps/', '');
      ...
      try {
        ...
        log.debug(`[NuxcoDepS] Not found, serving empty module: ${depFile}`);
        res.writeHead(200, {
          'Content-Type': 'application/javascript',
          'Cache-Control': 'no-cache, no-store, must-revalidate'
        });
        res.end(`// nuxco: empty stub for missing dep: ${depFile}\nexport default {};\n`);
      }
      return;
    }
    ...
    if (url === '/@nuxco/client') {
      ...
      try {
        ...
        try {
          ...
          res.writeHead(404);
          res.end('Nuxco client runtime not found');
        }
      }
      return;
    }

    if (url === '/@nuxco/error-overlay.js') {
      ...
      try {
        ...
        try {
          ...
          res.writeHead(404);
          res.end('Nuxco error overlay not found');
        }
      }
      return;
    }
    ...
    if (url === '/@nuxco/hmr-client' || url === '/@nuxco/hmr-client.js') {
      const srcPath = path.resolve(__dirname, '../../packages/nuxco-hmr-client/src/index.ts');
      try {
        ...
        log.warn(`[nuxco] /@nuxco/hmr-client: ${e.message}`);
        res.writeHead(404); res.end('/* @nuxco/hmr-client not built */');
      }
      return;
    }

    // Phase 3.5 — Module registry route
    if (url === '/@nuxco/module-registry' || url === '/@nuxco/module-registry.js') {
      const srcPath = path.resolve(__dirname, '../../packages/nuxco-module-registry/src/index.ts');
      try {
        const raw = await fs.readFile(srcPath, 'utf-8');
        const { transform } = await import('esbuild');
        const result = await transform(raw, { loader: 'ts', format: 'esm', target: 'es2020' });
        ...
        const initCall = Object.keys(remotes).length
          ? `\nimport{__nuxco_registry_init__ as _ri}from '/@nuxco/module-registry';_ri(${JSON.stringify(remotes)});\n`
          : '';
        res.writeHead(200, { 'Content-Type': 'application/javascript' });
        res.end(result.code + initCall);
      } catch (e: any) {
        log.warn(`[nuxco] /@nuxco/module-registry: ${e.message}`);
        res.writeHead(404); res.end('/* @nuxco/module-registry not built */');
      }
      return;
    }
    ...
    <script type="module">
      ...
      if (e.message && (e.frame || e.loc || e.stack)) {
        ...
        if (isJS && !res.headersSent) {
          ...
          res.writeHead(200, { 'Content-Type': 'application/javascript' });
          res.end(`
            console.error("[Nuxco Build Error]", ${JSON.stringify(e.message)});
            const error = ${JSON.stringify(errorData)};
            if (window.__NUXCO_ERROR_OVERLAY__) {
              window.__NUXCO_ERROR_OVERLAY__.show(error);
            }
            throw new Error("[Nuxco Build Error] See overlay for details");
          `);
          return;
        }
      }
  ...
  if (existingServer) {
    ...
    (server as any).__nuxco_handler = requestHandler;
  } else {
