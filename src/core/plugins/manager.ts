...
import { ZeptrPlugin, PluginHookName, PluginExecutionRecord, PluginValidation } from './types.js';
 ...
 * Plugin Manager
 * 
 * PUBLIC: Responsible for registering and executing plugins in the Zeptr pipeline.
...
export class PluginManager {
    /** @internal */
    private plugins: Map<string, ZeptrPlugin> = new Map();

    /** @public */
    async register(plugin: ZeptrPlugin | any) {
        // [SAFE REMOVAL] Phase 1.1: Wasmtime Sandbox removal
        if (plugin?.manifest?.type === 'wasm' || plugin?.path?.endsWith('.wasm')) {
            console.warn('[ZEPTR:WARN] WASM plugins are deprecated. See https://zeptr.dev/migrate for the new JS/TS Hook API hook migration path.');
            throw new Error(`[Zeptr] Error: .wasm plugin architecture has been removed for security and performance reasons. Plugin "${plugin?.manifest?.name || plugin?.name || 'unknown'}" must be migrated to a standard JS/TS hook format.`);
        }
    ...
    async registerWasmPlugin(wasmBytes: Buffer | Uint8Array | ArrayBuffer): Promise<void> {
        console.warn('[ZEPTR:WARN] registerWasmPlugin() is deprecated. See https://zeptr.dev/migrate');
        throw new Error('[Zeptr] Error: .wasm plugin architecture has been removed.');
    }
    ...
    async registerWasmPluginFromFile(filePath: string): Promise<void> {
        console.warn('[ZEPTR:WARN] registerWasmPluginFromFile() is deprecated. See https://zeptr.dev/migrate');
        throw new Error('[Zeptr] Error: .wasm plugin architecture has been removed.');
    }
    ...
    /** @internal */
    private hookCache: Map<string, ZeptrPlugin[]> = new Map();
