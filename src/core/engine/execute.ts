    ...
    for (const group of execPlan.parallelGroups) {
        const tasks = group.map(async (chunkId) => {
            ...
            if (!isCss) {
                ...
                bundleContent += isProd ? processShim + rt + fedRt : `/* Nuxc Runtime */\n${processShim}${rt}${fedRt}`;
            }
            ...
            // Phase 3.1 — Native DCE via nuxcChunk (production only, additive — falls back to existing logic)
            if (isProd && moduleResults.size > 0) {
                try {
                    ...
                    if (nat.nuxcChunk) {
                        ...
                        const result = nat.nuxcChunk(graphJson, {
                            strategy: 'auto',
                            maxChunkSizeKb: 0,
