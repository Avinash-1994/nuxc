    ...
    for (const group of execPlan.parallelGroups) {
        const tasks = group.map(async (chunkId) => {
            ...
            if (!isCss) {
                ...
                bundleContent += isProd ? processShim + rt + fedRt : `/* Nuxco Runtime */\n${processShim}${rt}${fedRt}`;
            }
            ...
            // Phase 3.1 — Native DCE via nuxcoChunk (production only, additive — falls back to existing logic)
            if (isProd && moduleResults.size > 0) {
                try {
                    ...
                    if (nat.nuxcoChunk) {
                        ...
                        const result = nat.nuxcoChunk(graphJson, {
                            strategy: 'auto',
                            maxChunkSizeKb: 0,
