...
async function main() {
  ...
  const argv = (yargs as any)(hideBin(process.argv))
    ...
    .fail((msg: string, err: Error, y: any) => {
      ...
      if (typed && !typed.startsWith('-')) {
        ...
        if (closest.d <= 3) {
          ...
          console.error(`Did you mean: nuxco ${closest.c} ?\n`);
          process.exit(1);
        }
      }
      console.error(`\n${msg}\n`);
    ...
    .command(
      'migrate',
      'Migrate config and plugins from older Nuxco versions',
      migrateCmd.options,
      migrateCmd.handler
    )
    ...
    .command(
      ...
      async (args: any) => {
        ...
        if (!graph) { console.error('No dependency graph — run nuxco build first.'); process.exit(1); }
    ...
    .command(
      ...
      async (args: any) => {
        ...
        let exitCode = 0;
        process.stderr.write('[nuxco:check] Resolving module graph...\n');
        const { FrameworkPipeline } = await import('./core/pipeline/framework-pipeline.js');
        const pipeline = await FrameworkPipeline.auto(config);
        if (!args['no-types']) {
          try {
            const { execSync } = await import('child_process');
            process.stderr.write('[nuxco:check] Running TypeScript type-check...\n');
            execSync('npx tsc --noEmit 2>&1', { cwd: process.cwd(), stdio: 'inherit' });
            process.stderr.write('[nuxco:check] TypeScript ✅\n');
          } catch { exitCode = 1; }
        }
        if (!args['no-circular']) {
          try {
            ...
            if (graph) {
              ...
              if (cycles.length > 0) {
                process.stderr.write(`[nuxco:check] ❌ ${cycles.length} circular import(s) detected:\n`);
                cycles.slice(0, 5).forEach(c => process.stderr.write(`  ${c.join(' → ')}\n`));
                exitCode = 1;
              } else {
                process.stderr.write('[nuxco:check] Circular imports ✅\n');
              }
            }
          } catch (e: any) { process.stderr.write(`[nuxco:check] Graph check skipped: ${e.message}\n`); }
        }
        process.stderr.write(exitCode === 0 ? '\n[nuxco:check] All checks passed ✅\n' : '\n[nuxco:check] Check failed ❌\n');
    ...
    .command(
      'test',
      'Run tests using Nuxco Custom Runner',
