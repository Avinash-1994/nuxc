 ...
 * Nuxc Astro Adapter
 *
 * Nuxc provides deeper Astro integration than any other build tool:
 * - Uses Astro's programmatic API to start the dev server
 * - Proxies all requests through Nuxc's security & HMR layer
 ...
 */
export class AstroAdapter implements NuxcAdapter {
  name = 'astro';
   ...
   * Nuxc wraps it with: HMR overlay, security headers, request timing, CSP.
   */
  getDevHandler(): any {
    ...
    const initAstro = async (root: string) => {
      ...
      try {
        // Resolve 'astro' from the PROJECT root, not Nuxc's own node_modules
        ...
        try {
          ...
          console.warn('[Nuxc:Astro] astro package not found in project node_modules. Run: npm install astro');
          return;
        }
        ...
        if (!astro || typeof astro.dev !== 'function') {
          console.warn('[Nuxc:Astro] astro.dev() not available — try updating astro to v4+');
          return;
        }
        ...
        const server = await astro.dev({
          ...
          // Integrate Nuxc's plugin pipeline
          vite: {
            plugins: [],
            define: { '__NUXC_BUILD__': 'true' }
          }
        });

        this._astroPort = internalPort;
        this._astroServer = server;
        console.log(`[Nuxc:Astro] ⚡ Astro dev server running internally on :${internalPort}`);
        ...
        proxyAgent.on('error', (err: Error, _req: any, res: any) => {
          console.error('[Nuxc:Astro] Proxy error:', err.message);
        ...
        console.error('[Nuxc:Astro] Failed to start Astro dev server:', e.message);
      }
    };

    return async (req: any, res: any, next: any) => {
      const root = req.__nuxcRoot || process.cwd();
