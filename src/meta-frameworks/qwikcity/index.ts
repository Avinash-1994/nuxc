/**
 * Sparx — Qwik City Meta-Framework Adapter (Phase 2.5)
 *
  ...
  for (const route of routes) {
    ...
    while ((sm = symbolPattern.exec(source)) !== null) {
      ...
      const code = [
        ...
        `// Route: ${route.path}`,
        `// Generated by Sparx Qwik Optimizer (Phase 2.5)`,
...
export function renderToString(opts: QwikSSROptions): string {
  ...
  return [
    ...
    `  <title>${title}</title>`,
    `  <meta name="description" content="Qwik City Store — Sparx Phase 2.5 SSR" />`,
    ...
    `    <a href="/" class="logo" q:id="logo">⚡ Sparx Qwik Store</a>`,
    `    <nav>`,
    ...
    `    <p>Powered by Sparx + Qwik City — Phase 2.5</p>`,
    `  </footer>`,
    `</div>`,
...
function _getTitle(url: string): string {
  if (url.includes('/products')) return 'Products | Sparx Qwik Store';
  if (url.includes('/cart')) return 'Cart | Sparx Qwik Store';
  if (url.includes('/about')) return 'About | Sparx Qwik Store';
  if (url.includes('/blog')) return 'Blog | Sparx Qwik Store';
  return 'Sparx Qwik Store';
}
...
function _buildPageContent(url: string, isAuthed: boolean): string {
  if (url.includes('/products')) {
    return [
      ...
      '          <h2>Sparx Pro Kit</h2><p class="price">$99.00</p>',
  ...
  return [
    ...
    '      <h1 q:id="hero-title">⚡ Sparx Qwik Store</h1>',
...
function _serializeState(url: string, isAuthed: boolean): Record<string, unknown> {
  return {
    ctx: {
      ...
      'hero': { visible: true },
      'hero-title': { text: 'Sparx Qwik Store' },
      'cart-total': { value: 0 },
      'products-section': { loaded: true },
    },
    refs: { currentUrl: url, isAuthed },
    objs: [url, isAuthed, 0, '⚡ Sparx Qwik Store'],
    subs: [],
  };
}
...
export async function executeServerAction(
  ...
  switch (actionName) {
    ...
    case 'subscribe': {
      ...
      return {
        status: 200,
        body: { success: true, email, message: 'Subscribed to Sparx newsletter' },
      };
    }
    default:
