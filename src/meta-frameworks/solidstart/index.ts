/**
 * Nuxc — SolidStart Streaming SSR Adapter (Phase 2.4)
 *
...
function buildShell(url: string, isAuthed: boolean): string {
  const title = url.includes('/dashboard') ? 'Dashboard | Nuxc SolidStart'
  ...
  return [
    ...
    '  <link rel="stylesheet" href="/_nuxc/assets/app.css" />',
    '</head>',
    '<body>',
    '<div id="root">',
    '  <header class="app-header">',
    '    <a href="/" class="logo">Nuxc Dashboard</a>',
    '    <nav>',
...
function buildDataChunk(url: string, isAuthed: boolean, elapsedMs: number): string {
  if (url.includes('/dashboard')) {
    return [
      ...
      '        <li><time>09:30</time> Deploy nuxc@1.0.10 → production</li>',
  ...
  if (url.includes('/products')) {
    return [
      ...
      '      <li><a href="/products/1">Nuxc Pro — $99/mo</a></li>',
      '      <li><a href="/products/2">Nuxc Team — $299/mo</a></li>',
      '      <li><a href="/products/3">Nuxc Enterprise — contact us</a></li>',
      '    </ul>',
    ].join('\n');
  }

  return '<h1>SolidStart SSR</h1><p>Rendered server-side by Nuxc.</p>';
}
 ...
 * Nuxc routes POST /_server/:actionId to this handler.
 */
...
export class SolidStartAdapter {
  ...
  createPlugin() {
    return {
      ...
      resolveId(id: string) {
        // Intercept 'solid-js/web' imports for Nuxc's own SSR shim
        if (id === 'solid-js/web') return '\0nuxc:solid-web-shim';
        return null;
      },

      load(id: string) {
        if (id === '\0nuxc:solid-web-shim') {
          return [
