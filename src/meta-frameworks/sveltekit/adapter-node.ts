import type { Middleware } from '@nuxc/adapter-core';
...
export function createSveltekitMiddleware(): Middleware {
  return async (req: any, res: any, next: any) => {
    ...
    try {
      ...
      const virtualModuleId = 'virtual:nuxc/sveltekit-routes';
      let manifest: any;
      try {
      ...
      // Map SvelteKit standard fetch Request / Response pattern over Nuxc's underlying uWS
      ...
      console.error('[Nuxc:SvelteKit] SSR Middleware Error', e);
      next();
    }
  };
}
