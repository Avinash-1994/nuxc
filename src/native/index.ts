...
function getJSFallback() {
    ...
    console.warn("⚠️  [NUCE EXECUTOR] Native binary and WASM failed to load. Falling back to JavaScript engine.");
    console.warn("⚠️  [NUCE EXECUTOR] Performance will be degraded. Execution is guaranteed iteratively safely.");
    return {
...
try {
    ...
    if (foundNative) {
        ...
        // See https://nuce.dev/migrate#wasm-plugins
        native = getJSFallback();
        engineUsed = 'js';
    }
} catch (e) {
    // If native or WASM crashes during load, catch and fallback to JS
    console.warn(`⚠️  [NUCE EXECUTOR] Engine failure: ${e instanceof Error ? e.message : String(e)}`);
    native = getJSFallback();
    engineUsed = 'js';
}
...
native.GraphAnalyzer = class DebugWrappedGraphAnalyzer extends OriginalGraphAnalyzer {
    ...
    addBatch(ids: string[], edges: string[][]) {
        if (process.env.NUCE_DEBUG_GRAPH) {
    ...
    detectCycles() {
        if (process.env.NUCE_DEBUG_GRAPH && typeof super.analyze !== 'function') {
    ...
    private _dumpSnapshot(entryPoints: string[]) {
        try {
            ...
            console.warn('⚠️  [NUCE DEBUG] Graph snapshot written to snapshot.json');
        } catch(e) {}
    }
};
