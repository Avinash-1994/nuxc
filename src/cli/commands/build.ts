...
async function printBuildSummary(outDir: string, elapsed: number) {
  ...
  const EXCLUDE_NAMES = ['nuce-sbom', 'nuce-csp', 'nuce-sri', 'nuce-headers', '_headers', '.htaccess'];
...
export default {
  ...
  handler: async (args: any) => {
    ...
    try {
      ...
      console.log('\n💡  Tip: Run `npx nuce preview` to serve the build locally.');
      console.log('💡  Tip: Run `npx nuce audit` to generate a full audit report.');
      ...
      if (args.watch) {
        ...
        const rebuild = async (filePath: string) => {
          console.log(`  [nuce] Changed: ${path.relative(root, filePath)}`);
