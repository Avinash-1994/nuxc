...
async function printBuildSummary(outDir: string, elapsed: number) {
  ...
  const EXCLUDE_NAMES = ['nuxco-sbom', 'nuxco-csp', 'nuxco-sri', 'nuxco-headers', '_headers', '.htaccess'];
...
export default {
  ...
  handler: async (args: any) => {
    ...
    try {
      ...
      console.log('\n💡  Tip: Run `npx nuxco preview` to serve the build locally.');
      console.log('💡  Tip: Run `npx nuxco audit` to generate a full audit report.');
      ...
      if (args.watch) {
        ...
        const rebuild = async (filePath: string) => {
          console.log(`  [nuxco] Changed: ${path.relative(root, filePath)}`);
