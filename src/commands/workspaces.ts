...
export async function startWorkspaceOrchestrator(rootDir: string) {
    console.log('\n🚀 Starting Nuxco Monorepo Workspace Orchestrator...');
        ...
        try {
            ...
            for (const entry of entries) {
                if (entry.isDirectory() && entry.name !== 'node_modules' && !entry.name.startsWith('.')) {
                    ...
                    try {
                        await fs.access(path.join(fullPath, 'nuxco.config.js'));
    ...
    if (projectPaths.length === 0) {
        console.error('❌ No Nuxco projects (with nuxco.config.js) found in subdirectories!');
        process.exit(1);
    }
