 ...
 * Day 17: Create-Nuxco Templates Lock
 */
...
// Read real nuxco version from its own package.json
function getNuxcoVersion(): string {
    try {
...
async function main() {
    ...
    if (!projectName || projectName.startsWith('--')) {
        console.error(red('Please specify project name: create-nuxco <name> or --name <name>'));
        process.exit(1);
    }
    ...
    const nuxcoVersion = getNuxcoVersion();
