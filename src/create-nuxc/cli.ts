 ...
 * Day 17: Create-Nuxc Templates Lock
 */
...
// Read real nuxc version from its own package.json
function getNuxcVersion(): string {
    try {
...
async function main() {
    ...
    if (!projectName || projectName.startsWith('--')) {
        console.error(red('Please specify project name: create-nuxc <name> or --name <name>'));
        process.exit(1);
    }
    ...
    const nuxcVersion = getNuxcVersion();
