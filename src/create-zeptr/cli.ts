 ...
 * Day 17: Create-Zeptr Templates Lock
 */
...
// Read real zeptr version from its own package.json
function getZeptrVersion(): string {
    try {
...
async function main() {
    ...
    if (!projectName || projectName.startsWith('--')) {
        console.error(red('Please specify project name: create-zeptr <name> or --name <name>'));
        process.exit(1);
    }
    ...
    const zeptrVersion = getZeptrVersion();
