 ...
 *
 * Bootstrap:
 *   The `__nuxc_registry_init__` script is injected once into the host page HTML.
 *   It creates globalThis.__nuxc_registry__ and wires the public API.
...
export class ModuleRegistry {
  ...
  private async _loadContainer(scope: string): Promise<RemoteContainer> {
    ...
    if (!entry) {
      throw new Error(
        `[nuxc:registry] Unknown scope "${scope}". ` +
  ...
  private _fetchContainer(scope: string, entry: RegistryEntry): Promise<RemoteContainer> {
    return new Promise<RemoteContainer>((resolve, reject) => {
      ...
      script.dataset.nuxcScope = scope;

      script.onload = () => {
        const container =
          (globalThis as any)[`nuxc_remote_${scope}`] ??
          (globalThis as any)[scope];

        if (!container) {
          reject(
            new Error(
              `[nuxc:registry] Remote "${scope}" loaded from ${entry.url} ` +
              `but no container found on globalThis. ` +
              `Ensure the remote was built with Nuxc MF.`
            )
          );
          return;
        }
      ...
      script.onerror = () => {
        reject(
          new Error(`[nuxc:registry] Failed to load remote "${scope}" from ${entry.url}`)
        );
      };
  ...
  private _initContainer(scope: string, container: RemoteContainer): void {
    ...
    (globalThis as any)[`nuxc_remote_${scope}`] = container;
  }
  ...
  private _waitForContainer(
    ...
    if (attempts > 50) {
      reject(new Error(`[nuxc:registry] Timeout waiting for remote "${scope}" container`));
      return;
    }
...
export function getGlobalRegistry(): ModuleRegistry {
  ...
  if (!g.__nuxc_registry__) {
    g.__nuxc_registry__ = new ModuleRegistry(g.__nuxc_shared__ ?? {});
    // Flush any calls buffered before the registry was ready
    if (Array.isArray(g.__nuxc_registry_queue__)) {
      for (const { method, args } of g.__nuxc_registry_queue__) {
        (g.__nuxc_registry__ as any)[method]?.(...args);
      }
      g.__nuxc_registry_queue__ = [];
    }
  }
  return g.__nuxc_registry__;
}
