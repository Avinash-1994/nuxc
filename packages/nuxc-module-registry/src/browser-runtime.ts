/**
 * @nuxc/module-registry — Browser Runtime
 *
 * This module generates the `__nuxc_registry_init__` inline script
 * that must be injected into the host page <head> BEFORE any remote
 * scripts are loaded.
 *
 * It does three things:
 *   1. Creates `globalThis.__nuxc_registry__` (the ModuleRegistry singleton)
...
export function generateRegistryInitScript(): string {
  return `
// Nuxc Module Registry — __nuxc_registry_init__
// Phase 1.11 | https://nuxc.dev/docs/module-registry
(function() {
  if (typeof globalThis.__nuxc_registry__ !== 'undefined') return;

  // ── Queue: buffer early calls before registry is ready ──────────────────────
  var _queue = globalThis.__nuxc_registry_queue__ = globalThis.__nuxc_registry_queue__ || [];

  // ── Internal registry map: scope → entry ────────────────────────────────────
  var _entries = {};          // { [scope]: { url, state, container, promise } }
  var _sharedScope = globalThis.__nuxc_shared__ || {};
  ...
  function _fetch(scope) {
    ...
    if (!entry) return Promise.reject(new Error('[nuxc:registry] Unknown scope: ' + scope));
    if (entry.state === 'ready')   return Promise.resolve(entry.container);
    if (entry.state === 'loading') return entry.promise;

    var promise = new Promise(function(resolve, reject) {
      var existing = document.querySelector('script[data-nuxc-scope="' + scope + '"]');
      if (!existing) {
        var s = document.createElement('script');
        s.type = 'module';
        s.crossOrigin = 'anonymous';
        s.src = entry.url;
        s.dataset.nuxcScope = scope;
        s.onload = function() {
          var c = globalThis['nuxc_remote_' + scope] || globalThis[scope];
          if (!c) {
            return reject(new Error('[nuxc:registry] Remote "' + scope + '" has no container on globalThis after load.'));
          }
        ...
        s.onerror = function() {
          ...
          reject(new Error('[nuxc:registry] Failed to load remote "' + scope + '" from ' + entry.url));
        };
        ...
        (function poll() {
          ...
          if (++attempts > 50) return reject(new Error('[nuxc:registry] Timeout for scope "' + scope + '"'));
          setTimeout(poll, 100);
        })();
      }
    });
  ...
  globalThis.__nuxc_registry__ = _registry;

  // Expose top-level helpers for non-bundled callers
  globalThis.__nuxc_register__   = function(s, u) { return _registry.register(s, u); };
  globalThis.__nuxc_load__       = function(s, m) { return _registry.load(s, m); };
  globalThis.__nuxc_invalidate__ = function(s)    { return _registry.invalidate(s); };
  globalThis.__nuxc_preload__    = function(s)    { return _registry.preload(s); };
...
export function injectRegistryIntoHTML(html: string): string {
  if (html.includes('__nuxc_registry_init__')) return html; // already injected
