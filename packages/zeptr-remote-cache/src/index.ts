 ...
 *
 * Interfaces with S3 or Zeptr Cloud to fetch/store build artifacts.
 ...
 *  - zeptr-cloud: Zeptr Cloud managed cache (requires token)
 *
 * Options:
...
export class ZeptrCloudProvider implements RemoteCacheProvider {
  constructor(private options: RemoteCacheOptions) {}
  ...
  private getCloudUrl(key: string): string {
    const baseUrl = this.options.baseUrl || 'https://cache.zeptr.dev/v1';
...
export function createRemoteCache(options: RemoteCacheOptions | boolean): RemoteCacheProvider {
  ...
  const opts = typeof options === 'boolean' ? { provider: 'zeptr-cloud' as const } : options;

  switch (opts.provider) {
    case 's3':
      return new S3Provider(opts);
    case 'zeptr-cloud':
      return new ZeptrCloudProvider(opts);
    default:
      return new NullProvider();
  }
}
