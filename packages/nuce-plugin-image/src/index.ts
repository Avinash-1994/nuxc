/**
 * @nuce/plugin-image — Image Optimization via sharp
...
const PLUGIN_PERMISSIONS = {
  nuce: { permissions: ['fs:read', 'fs:write'] }
};
 ...
 * Main plugin factory.
 */
export function nucePluginImage(options: ImagePluginOptions = {}): any {
  ...
  return {
    ...
    async buildStart() {
      ...
      if (!sharpLib) {
        console.info(
          '[nuce:plugin-image] INFO: sharp not installed — using copy-only fallback.\n' +
    ...
    async buildOutput(outputDir: string): Promise<void> {
      ...
      console.log(`[nuce:plugin-image] Processing ${imageFiles.length} image(s)...`);
      ...
      console.log(`[nuce:plugin-image] Generated ${total} variant(s) in ${ms}ms`);
    },
...
export default nucePluginImage;
