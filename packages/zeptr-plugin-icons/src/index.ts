/**
 * @zeptr/plugin-icons — 100,000+ Icons, Zero Bundle Bloat
...
export interface IconsPluginOptions {
  ...
  /** Cache directory (default: .zeptr/cache/icons) */
  cacheDir?: string;
}
...
export function icons(options: IconsPluginOptions = {}) {
  const {
    compiler = 'react',
    defaultClass = 'zeptr-icon',
    cacheDir = '.zeptr/cache/icons',
    customCollections = {},
  } = options;
  ...
  return {
    ...
    async load(id: string) {
      ...
      if (!svg) {
        console.warn(`[zeptr:icons] Icon not found: ${collection}/${name}`);
