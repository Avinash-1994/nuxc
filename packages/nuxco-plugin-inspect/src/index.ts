/**
 * @nuxco/plugin-inspect — See exactly what every plugin does to your modules
...
export interface InspectOptions {
  /** URL path for the inspect UI (default: '/__nuxco_inspect__') */
  basePath?: string;
}

export function inspect(options: InspectOptions = {}) {
  if (process.env['NODE_ENV'] === 'production') {
    return { name: '@nuxco/plugin-inspect' }; // No-op in production
  }

  const { basePath = '/__nuxco_inspect__' } = options;
  ...
  return {
    ...
    configureServer(server: any) {
      ...
      console.info(`[nuxco:inspect] 🔍 Inspect UI: http://localhost:5173${basePath}`);
    },
