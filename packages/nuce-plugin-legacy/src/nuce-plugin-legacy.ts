/**
 * @nuce/plugin-legacy — Support older browsers without hurting modern ones
...
function generatePolyfillBundle(): string {
  return `// @nuce/plugin-legacy polyfills
// Object.assign
...
// Promise (minimal)
if (typeof Promise === 'undefined') { console.warn('[nuce:legacy] Promise polyfill not included — use core-js'); }
// Array.from
...
export function legacy(options: LegacyOptions = {}) {
  ...
  return {
    ...
    async closeBundle() {
      ...
      console.info(`[nuce:legacy] ✅ Legacy bundle support injected (Safari 10 fix: ${safari10})`);
    },
  };
}
