/**
 * @nuce/plugin-legacy — Legacy Browser Support via SWC + core-js injection
 ...
 * In production this calls nuce-native; in test environments it uses
...
async function downlevelWithSwc(code: string, targets: string[]): Promise<string> {
  try {
    ...
    // @ts-ignore — nuce-native is optional in test environments
...
/**
 * Main plugin factory — call with options, register in nuce.config.ts plugins[]
 */
export function nucePluginLegacy(options: LegacyPluginOptions = {}): any {
  const {
  ...
  return {
    ...
    // Nuce build hook: write legacy files after main bundle
...
export default nucePluginLegacy;
