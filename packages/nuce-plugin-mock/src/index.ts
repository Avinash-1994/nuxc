/**
 * @nuce/plugin-mock — Mock API responses in dev, zero backend needed
...
export function mock(options: MockPluginOptions = {}) {
  ...
  if (!enable) {
    return { name: '@nuce/plugin-mock', configureServer() {} };
  }
  ...
  return {
    ...
    configResolved(config: any) {
      ...
      console.info(`[nuce:mock] Loaded ${handlers.size} mock route(s) from ${mockDir}/`);
    },
      ...
      if (server.watcher) {
        ...
        server.watcher.on('change', (file: string) => {
          if (file.startsWith(mockDir)) {
            handlers = loadMockHandlers(mockDir);
            console.info('[nuce:mock] Mock handlers reloaded.');
          }
        });
      }
      ...
      server.middlewares?.use(async (req: any, res: any, next: () => void) => {
        ...
        req.on('end', async () => {
          ...
          try {
            ...
            console.error('[nuce:mock] Handler error:', err);
            next();
          }
        });
      });
