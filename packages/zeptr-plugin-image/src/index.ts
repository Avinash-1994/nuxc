/**
 * @zeptr/plugin-image — Image Optimization via sharp
...
const PLUGIN_PERMISSIONS = {
  zeptr: { permissions: ['fs:read', 'fs:write'] }
};
 ...
 * Main plugin factory.
 */
export function zeptrPluginImage(options: ImagePluginOptions = {}): any {
  ...
  return {
    ...
    async buildStart() {
      ...
      if (!sharpLib) {
        console.info(
          '[zeptr:plugin-image] INFO: sharp not installed — using copy-only fallback.\n' +
    ...
    async buildOutput(outputDir: string): Promise<void> {
      ...
      console.log(`[zeptr:plugin-image] Processing ${imageFiles.length} image(s)...`);
      ...
      console.log(`[zeptr:plugin-image] Generated ${total} variant(s) in ${ms}ms`);
    },
...
export default zeptrPluginImage;
