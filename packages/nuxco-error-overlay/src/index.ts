 ...
 *
 * Provides a rich, browser-rendered error overlay for the Nuxco dev server:
...
export function formatOverlayTitle(err: NuxcoError): string {
  return KIND_LABELS[err.kind] ?? 'Build Error';
}

export function formatOverlaySubtitle(err: NuxcoError): string {
  return `${err.location.file}:${err.location.line}${err.location.column != null ? `:${err.location.column}` : ''}`;
}

// ─── HTML overlay renderer ────────────────────────────────────────────────────

export function renderOverlayHTML(err: NuxcoError): string {
  ...
  const codeFrame = err.codeFrame
    ? `<pre class="nuxco-code-frame">${escapeHtml(err.codeFrame)}</pre>`
    : '';
  ...
  return `
<div id="nuxco-error-overlay" style="
  ...
  <div style="
    ...
    <div style="margin-top:16px;display:flex;gap:12px;flex-wrap:wrap;">
      ...
      <button onclick="document.getElementById('nuxco-error-overlay').remove()" style="
...
(function() {
  if (typeof window.__nuxco_hmr__ === 'undefined') return;
  window.__nuxco_hmr__.on('update', function() {
    var overlay = document.getElementById('nuxco-error-overlay');
    if (overlay) {
...
export function createOverlay(rawError: {
  ...
  const err: NuxcoError = { kind, message: rawError.message, location, codeFrame: rawError.codeFrame, stack: rawError.stack };
  return {
