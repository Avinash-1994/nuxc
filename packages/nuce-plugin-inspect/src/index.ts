/**
 * @nuce/plugin-inspect — See exactly what every plugin does to your modules
...
export interface InspectOptions {
  /** URL path for the inspect UI (default: '/__nuce_inspect__') */
  basePath?: string;
}

export function inspect(options: InspectOptions = {}) {
  if (process.env['NODE_ENV'] === 'production') {
    return { name: '@nuce/plugin-inspect' }; // No-op in production
  }

  const { basePath = '/__nuce_inspect__' } = options;
  ...
  return {
    ...
    configureServer(server: any) {
      ...
      console.info(`[nuce:inspect] 🔍 Inspect UI: http://localhost:5173${basePath}`);
    },
