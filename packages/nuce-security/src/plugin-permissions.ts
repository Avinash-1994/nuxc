 ...
 * Plugins must declare permissions in package.json under nuce.permissions.
 */
...
function logViolation(pluginName: string, operation: string, logDir?: string): void {
  ...
  console.warn(`[nuce:security] ${msg}`);
  if (logDir) {
...
export function loadPluginPermissions(
  ...
  try {
    ...
    const declared: PluginPermission[] = pkg?.nuce?.permissions ?? [];
    ...
    get(target, prop, receiver) {
      ...
      if (prop === 'env' || prop === 'process') {
        if (!declared.includes('env:read')) {
          ...
          if (mode === 'production') {
            throw new Error(`[nuce:security] Plugin "${name}" requires env:read permission.`);
          }
...
/** List installed Nuce plugins with their declared permissions for audit. */
