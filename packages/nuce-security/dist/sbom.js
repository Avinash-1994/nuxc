 ...
 * Generates dist/nuce-sbom.json listing every npm + Rust dep
 ...
 * @param projectRoot – root of the Nuce project
...
/** Write the SBOM to dist/nuce-sbom.json */
export function writeSBOM(report, distDir) {
    const outPath = path.join(distDir, 'nuce-sbom.json');
    fs.mkdirSync(distDir, { recursive: true });
    fs.writeFileSync(outPath, JSON.stringify(report, null, 2), 'utf8');
    console.info(`[nuce:security] SBOM written → ${outPath} (${report.components.length} components)`);
}
