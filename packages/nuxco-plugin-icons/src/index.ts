/**
 * @nuxco/plugin-icons — 100,000+ Icons, Zero Bundle Bloat
...
export interface IconsPluginOptions {
  ...
  /** Cache directory (default: .nuxco/cache/icons) */
  cacheDir?: string;
}
...
export function icons(options: IconsPluginOptions = {}) {
  const {
    compiler = 'react',
    defaultClass = 'nuxco-icon',
    cacheDir = '.nuxco/cache/icons',
    customCollections = {},
  } = options;
  ...
  return {
    ...
    async load(id: string) {
      ...
      if (!svg) {
        console.warn(`[nuxco:icons] Icon not found: ${collection}/${name}`);
