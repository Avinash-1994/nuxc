...
export function checker(options: CheckerOptions = {}) {
  ...
  async function runChecks(isProduction: boolean): Promise<string[]> {
    ...
    if (typescript) {
      ...
      if (errors.length) console.error(`[nuxc:checker] TypeScript: ${errors.length} error(s)`);
    }
    ...
    if (eslint && changedFiles.size > 0) {
      ...
      allErrors.push(...errors);
      if (errors.length) console.error(`[nuxc:checker] ESLint: ${errors.length} issue(s)`);
    }
  ...
  return {
    ...
    async buildStart() {
      ...
      if (errors.length > 0) {
        throw new Error(
          `[nuxc:checker] Build aborted: ${errors.length} type/lint error(s) found.\n` +
