// packages/nuce-adapter-core/src/registry.ts
import type { NuceAdapter, PackageJson } from './index.js';

export class AdapterRegistry {
  private adapters: NuceAdapter[] = [];

  register(adapter: NuceAdapter) {
  ...
  private activeAdapter: NuceAdapter | null = null;

  getActiveAdapter(): NuceAdapter | null {
    return this.activeAdapter;
  }

  detect(projectRoot: string, pkg: PackageJson): NuceAdapter | null {
    ...
    for (const adapter of sortedAdapters) {
      if (adapter.detect(projectRoot, pkg)) {
        if (adapter.name === 'gatsby') {
          console.warn('[NUCE:INFO] Gatsby adapter is currently in community preview. Report issues at https://nuce.dev/issues');
        }
