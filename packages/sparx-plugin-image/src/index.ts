/**
 * @sparx/plugin-image — Image Optimization via sharp
...
const PLUGIN_PERMISSIONS = {
  sparx: { permissions: ['fs:read', 'fs:write'] }
};
 ...
 * Main plugin factory.
 */
export function sparxPluginImage(options: ImagePluginOptions = {}): any {
  ...
  return {
    ...
    async buildStart() {
      ...
      if (!sharpLib) {
        console.info(
          '[sparx:plugin-image] INFO: sharp not installed — using copy-only fallback.\n' +
    ...
    async buildOutput(outputDir: string): Promise<void> {
      ...
      console.log(`[sparx:plugin-image] Processing ${imageFiles.length} image(s)...`);
      ...
      console.log(`[sparx:plugin-image] Generated ${total} variant(s) in ${ms}ms`);
    },
...
export default sparxPluginImage;
