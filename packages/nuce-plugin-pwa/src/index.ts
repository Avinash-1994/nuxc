/**
 * @nuce/plugin-pwa — Progressive Web App, one plugin, full offline support
...
function generateServiceWorker(precacheUrls: string[], options: PWAOptions): string {
  ...
  return `// @nuce/plugin-pwa — Generated Service Worker
const CACHE_VERSION = 'nuce-v${cacheVersion}';
...
export function pwa(options: PWAOptions) {
  ...
  return {
    ...
    async closeBundle() {
      ...
      console.info(`[nuce:pwa] ✅ PWA manifest + service worker generated (${assets.length} assets precached)`);
    },
