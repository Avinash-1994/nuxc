/**
 * @sparx/plugin-legacy — Legacy Browser Support via SWC + core-js injection
 ...
 * In production this calls sparx-native; in test environments it uses
...
async function downlevelWithSwc(code: string, targets: string[]): Promise<string> {
  try {
    ...
    // @ts-ignore — sparx-native is optional in test environments
...
/**
 * Main plugin factory — call with options, register in sparx.config.ts plugins[]
 */
export function sparxPluginLegacy(options: LegacyPluginOptions = {}): any {
  const {
  ...
  return {
    ...
    // Sparx build hook: write legacy files after main bundle
...
export default sparxPluginLegacy;
