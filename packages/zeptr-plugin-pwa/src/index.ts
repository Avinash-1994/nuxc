/**
 * @zeptr/plugin-pwa — Progressive Web App, one plugin, full offline support
...
function generateServiceWorker(precacheUrls: string[], options: PWAOptions): string {
  ...
  return `// @zeptr/plugin-pwa — Generated Service Worker
const CACHE_VERSION = 'zeptr-v${cacheVersion}';
...
export function pwa(options: PWAOptions) {
  ...
  return {
    ...
    async closeBundle() {
      ...
      console.info(`[zeptr:pwa] ✅ PWA manifest + service worker generated (${assets.length} assets precached)`);
    },
