/**
 * @nuxco/plugin-image — Image Optimization via sharp
...
const PLUGIN_PERMISSIONS = {
  nuxco: { permissions: ['fs:read', 'fs:write'] }
};
 ...
 * Main plugin factory.
 */
export function nuxcoPluginImage(options: ImagePluginOptions = {}): any {
  ...
  return {
    ...
    async buildStart() {
      ...
      if (!sharpLib) {
        console.info(
          '[nuxco:plugin-image] INFO: sharp not installed — using copy-only fallback.\n' +
    ...
    async buildOutput(outputDir: string): Promise<void> {
      ...
      console.log(`[nuxco:plugin-image] Processing ${imageFiles.length} image(s)...`);
      ...
      console.log(`[nuxco:plugin-image] Generated ${total} variant(s) in ${ms}ms`);
    },
...
export default nuxcoPluginImage;
