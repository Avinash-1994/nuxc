 ...
 * Emits dist/nuxco-headers.json with recommended server headers
...
export function generateSecurityHeaders(
  ...
  const nginx = `# Nuxco Security Headers — Nginx
...
add_header Content-Security-Policy "${csp}" always;`;
  ...
  const apache = `# Nuxco Security Headers — Apache
...
Header always set Content-Security-Policy "${csp}"`;
  ...
  const netlify = `# Nuxco Security Headers — Netlify (_headers file)
/*
${headerLines.map(([k, v]) => `  ${k}: ${v}`).join('\n')}`;

  const cloudflare = `// Nuxco Security Headers — Cloudflare Workers
...
export default {
  ...
  if (fs.existsSync(distDir)) {
    fs.writeFileSync(
      path.join(distDir, 'nuxco-headers.json'),
      JSON.stringify(result, null, 2),
      'utf8'
    );
    console.info(`[nuxco:security] Security headers → ${path.join(distDir, 'nuxco-headers.json')}`);
  }
