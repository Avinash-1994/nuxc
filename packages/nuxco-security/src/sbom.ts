 ...
 * Generates dist/nuxco-sbom.json listing every npm + Rust dep
 ...
 * @param projectRoot – root of the Nuxco project
...
/** Write the SBOM to dist/nuxco-sbom.json */
export function writeSBOM(report: SBOMReport, distDir: string): void {
  const outPath = path.join(distDir, 'nuxco-sbom.json');
  fs.mkdirSync(distDir, { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(report, null, 2), 'utf8');
  console.info(`[nuxco:security] SBOM written → ${outPath} (${report.components.length} components)`);
}
