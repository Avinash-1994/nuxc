 ...
 * Plugins must declare permissions in package.json under nuxco.permissions.
 */
...
function logViolation(pluginName: string, operation: string, logDir?: string): void {
  ...
  console.warn(`[nuxco:security] ${msg}`);
  if (logDir) {
...
export function loadPluginPermissions(
  ...
  try {
    ...
    const declared: PluginPermission[] = pkg?.nuxco?.permissions ?? [];
    ...
    get(target, prop, receiver) {
      ...
      if (prop === 'env' || prop === 'process') {
        if (!declared.includes('env:read')) {
          ...
          if (mode === 'production') {
            throw new Error(`[nuxco:security] Plugin "${name}" requires env:read permission.`);
          }
...
/** List installed Nuxco plugins with their declared permissions for audit. */
