/**
 * @zeptr/plugin-inspect — See exactly what every plugin does to your modules
...
export interface InspectOptions {
  /** URL path for the inspect UI (default: '/__zeptr_inspect__') */
  basePath?: string;
}

export function inspect(options: InspectOptions = {}) {
  if (process.env['NODE_ENV'] === 'production') {
    return { name: '@zeptr/plugin-inspect' }; // No-op in production
  }

  const { basePath = '/__zeptr_inspect__' } = options;
  ...
  return {
    ...
    configureServer(server: any) {
      ...
      console.info(`[zeptr:inspect] 🔍 Inspect UI: http://localhost:5173${basePath}`);
    },
