 ...
 *
 * Interfaces with S3 or Nuxco Cloud to fetch/store build artifacts.
 ...
 *  - nuxco-cloud: Nuxco Cloud managed cache (requires token)
 *
 * Options:
...
export class NuxcoCloudProvider implements RemoteCacheProvider {
  constructor(private options: RemoteCacheOptions) {}
  ...
  private getCloudUrl(key: string): string {
    const baseUrl = this.options.baseUrl || 'https://cache.nuxco.dev/v1';
...
export function createRemoteCache(options: RemoteCacheOptions | boolean): RemoteCacheProvider {
  ...
  const opts = typeof options === 'boolean' ? { provider: 'nuxco-cloud' as const } : options;

  switch (opts.provider) {
    case 's3':
      return new S3Provider(opts);
    case 'nuxco-cloud':
      return new NuxcoCloudProvider(opts);
    default:
      return new NullProvider();
  }
}
