 ...
 *
 * Provides a rich, browser-rendered error overlay for the Nuxc dev server:
...
export function formatOverlayTitle(err: NuxcError): string {
  return KIND_LABELS[err.kind] ?? 'Build Error';
}

export function formatOverlaySubtitle(err: NuxcError): string {
  return `${err.location.file}:${err.location.line}${err.location.column != null ? `:${err.location.column}` : ''}`;
}

// ─── HTML overlay renderer ────────────────────────────────────────────────────

export function renderOverlayHTML(err: NuxcError): string {
  ...
  const codeFrame = err.codeFrame
    ? `<pre class="nuxc-code-frame">${escapeHtml(err.codeFrame)}</pre>`
    : '';
  ...
  return `
<div id="nuxc-error-overlay" style="
  ...
  <div style="
    ...
    <div style="margin-top:16px;display:flex;gap:12px;flex-wrap:wrap;">
      ...
      <button onclick="document.getElementById('nuxc-error-overlay').remove()" style="
...
(function() {
  if (typeof window.__nuxc_hmr__ === 'undefined') return;
  window.__nuxc_hmr__.on('update', function() {
    var overlay = document.getElementById('nuxc-error-overlay');
    if (overlay) {
...
export function createOverlay(rawError: {
  ...
  const err: NuxcError = { kind, message: rawError.message, location, codeFrame: rawError.codeFrame, stack: rawError.stack };
  return {
