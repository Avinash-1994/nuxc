/**
 * @zeptr/module-registry — Browser Runtime
 *
 * This module generates the `__zeptr_registry_init__` inline script
 * that must be injected into the host page <head> BEFORE any remote
 * scripts are loaded.
 *
 * It does three things:
 *   1. Creates `globalThis.__zeptr_registry__` (the ModuleRegistry singleton)
...
export function generateRegistryInitScript(): string {
  return `
// Zeptr Module Registry — __zeptr_registry_init__
// Phase 1.11 | https://zeptr.dev/docs/module-registry
(function() {
  if (typeof globalThis.__zeptr_registry__ !== 'undefined') return;

  // ── Queue: buffer early calls before registry is ready ──────────────────────
  var _queue = globalThis.__zeptr_registry_queue__ = globalThis.__zeptr_registry_queue__ || [];

  // ── Internal registry map: scope → entry ────────────────────────────────────
  var _entries = {};          // { [scope]: { url, state, container, promise } }
  var _sharedScope = globalThis.__zeptr_shared__ || {};
  ...
  function _fetch(scope) {
    ...
    if (!entry) return Promise.reject(new Error('[zeptr:registry] Unknown scope: ' + scope));
    if (entry.state === 'ready')   return Promise.resolve(entry.container);
    if (entry.state === 'loading') return entry.promise;

    var promise = new Promise(function(resolve, reject) {
      var existing = document.querySelector('script[data-zeptr-scope="' + scope + '"]');
      if (!existing) {
        var s = document.createElement('script');
        s.type = 'module';
        s.crossOrigin = 'anonymous';
        s.src = entry.url;
        s.dataset.zeptrScope = scope;
        s.onload = function() {
          var c = globalThis['zeptr_remote_' + scope] || globalThis[scope];
          if (!c) {
            return reject(new Error('[zeptr:registry] Remote "' + scope + '" has no container on globalThis after load.'));
          }
        ...
        s.onerror = function() {
          ...
          reject(new Error('[zeptr:registry] Failed to load remote "' + scope + '" from ' + entry.url));
        };
        ...
        (function poll() {
          ...
          if (++attempts > 50) return reject(new Error('[zeptr:registry] Timeout for scope "' + scope + '"'));
          setTimeout(poll, 100);
        })();
      }
    });
  ...
  globalThis.__zeptr_registry__ = _registry;

  // Expose top-level helpers for non-bundled callers
  globalThis.__zeptr_register__   = function(s, u) { return _registry.register(s, u); };
  globalThis.__zeptr_load__       = function(s, m) { return _registry.load(s, m); };
  globalThis.__zeptr_invalidate__ = function(s)    { return _registry.invalidate(s); };
  globalThis.__zeptr_preload__    = function(s)    { return _registry.preload(s); };
...
export function injectRegistryIntoHTML(html: string): string {
  if (html.includes('__zeptr_registry_init__')) return html; // already injected
