/**
 * @nuxco/module-registry — Browser Runtime
 *
 * This module generates the `__nuxco_registry_init__` inline script
 * that must be injected into the host page <head> BEFORE any remote
 * scripts are loaded.
 *
 * It does three things:
 *   1. Creates `globalThis.__nuxco_registry__` (the ModuleRegistry singleton)
...
export function generateRegistryInitScript(): string {
  return `
// Nuxco Module Registry — __nuxco_registry_init__
// Phase 1.11 | https://nuxco.dev/docs/module-registry
(function() {
  if (typeof globalThis.__nuxco_registry__ !== 'undefined') return;

  // ── Queue: buffer early calls before registry is ready ──────────────────────
  var _queue = globalThis.__nuxco_registry_queue__ = globalThis.__nuxco_registry_queue__ || [];

  // ── Internal registry map: scope → entry ────────────────────────────────────
  var _entries = {};          // { [scope]: { url, state, container, promise } }
  var _sharedScope = globalThis.__nuxco_shared__ || {};
  ...
  function _fetch(scope) {
    ...
    if (!entry) return Promise.reject(new Error('[nuxco:registry] Unknown scope: ' + scope));
    if (entry.state === 'ready')   return Promise.resolve(entry.container);
    if (entry.state === 'loading') return entry.promise;

    var promise = new Promise(function(resolve, reject) {
      var existing = document.querySelector('script[data-nuxco-scope="' + scope + '"]');
      if (!existing) {
        var s = document.createElement('script');
        s.type = 'module';
        s.crossOrigin = 'anonymous';
        s.src = entry.url;
        s.dataset.nuxcoScope = scope;
        s.onload = function() {
          var c = globalThis['nuxco_remote_' + scope] || globalThis[scope];
          if (!c) {
            return reject(new Error('[nuxco:registry] Remote "' + scope + '" has no container on globalThis after load.'));
          }
        ...
        s.onerror = function() {
          ...
          reject(new Error('[nuxco:registry] Failed to load remote "' + scope + '" from ' + entry.url));
        };
        ...
        (function poll() {
          ...
          if (++attempts > 50) return reject(new Error('[nuxco:registry] Timeout for scope "' + scope + '"'));
          setTimeout(poll, 100);
        })();
      }
    });
  ...
  globalThis.__nuxco_registry__ = _registry;

  // Expose top-level helpers for non-bundled callers
  globalThis.__nuxco_register__   = function(s, u) { return _registry.register(s, u); };
  globalThis.__nuxco_load__       = function(s, m) { return _registry.load(s, m); };
  globalThis.__nuxco_invalidate__ = function(s)    { return _registry.invalidate(s); };
  globalThis.__nuxco_preload__    = function(s)    { return _registry.preload(s); };
...
export function injectRegistryIntoHTML(html: string): string {
  if (html.includes('__nuxco_registry_init__')) return html; // already injected
