 ...
 * Emits dist/zeptr-headers.json with recommended server headers
...
export function generateSecurityHeaders(
  ...
  const nginx = `# Zeptr Security Headers — Nginx
...
add_header Content-Security-Policy "${csp}" always;`;
  ...
  const apache = `# Zeptr Security Headers — Apache
...
Header always set Content-Security-Policy "${csp}"`;
  ...
  const netlify = `# Zeptr Security Headers — Netlify (_headers file)
/*
${headerLines.map(([k, v]) => `  ${k}: ${v}`).join('\n')}`;

  const cloudflare = `// Zeptr Security Headers — Cloudflare Workers
...
export default {
  ...
  if (fs.existsSync(distDir)) {
    fs.writeFileSync(
      path.join(distDir, 'zeptr-headers.json'),
      JSON.stringify(result, null, 2),
      'utf8'
    );
    console.info(`[zeptr:security] Security headers → ${path.join(distDir, 'zeptr-headers.json')}`);
  }
