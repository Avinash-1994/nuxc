 ...
 * Plugins must declare permissions in package.json under zeptr.permissions.
 */
...
function logViolation(pluginName: string, operation: string, logDir?: string): void {
  ...
  console.warn(`[zeptr:security] ${msg}`);
  if (logDir) {
...
export function loadPluginPermissions(
  ...
  try {
    ...
    const declared: PluginPermission[] = pkg?.zeptr?.permissions ?? [];
    ...
    get(target, prop, receiver) {
      ...
      if (prop === 'env' || prop === 'process') {
        if (!declared.includes('env:read')) {
          ...
          if (mode === 'production') {
            throw new Error(`[zeptr:security] Plugin "${name}" requires env:read permission.`);
          }
...
/** List installed Zeptr plugins with their declared permissions for audit. */
