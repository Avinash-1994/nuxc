 ...
 * Generates dist/zeptr-sbom.json listing every npm + Rust dep
 ...
 * @param projectRoot – root of the Zeptr project
...
/** Write the SBOM to dist/zeptr-sbom.json */
export function writeSBOM(report: SBOMReport, distDir: string): void {
  const outPath = path.join(distDir, 'zeptr-sbom.json');
  fs.mkdirSync(distDir, { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(report, null, 2), 'utf8');
  console.info(`[zeptr:security] SBOM written → ${outPath} (${report.components.length} components)`);
}
