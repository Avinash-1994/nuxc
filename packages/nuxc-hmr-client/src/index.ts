...
declare const __NUXC_HMR_URL__: string | undefined;

function resolveHmrUrl(): string {
  if (typeof __NUXC_HMR_URL__ !== 'undefined') return __NUXC_HMR_URL__;
  if (typeof import.meta !== 'undefined' && (import.meta as any).env?.NUXC_HMR_URL) {
    return (import.meta as any).env.NUXC_HMR_URL;
  }
  const proto = location.protocol === 'https:' ? 'wss:' : 'ws:';
  return `${proto}//${location.host}/__nuxc_hmr`;
}
...
function stamp(key: string) {
  (window as any).__nuxcHmr = (window as any).__nuxcHmr ?? {};
  (window as any).__nuxcHmr[key] = Date.now();
}
  ...
  for (const mod of modules) {
    ...
    if (cbs.length === 0) {
      ...
      try {
        ...
        console.error('[nuxc:hmr] Failed to hot-update', mod, err);
        handleFullReload();
        return;
      }
    }
...
function onMessage(raw: string): void {
  ...
  switch (msg.type) {
    case 'connected':
      console.debug('[nuxc:hmr] connected');
      reconnectDelay = 1000;
      stamp('connected');
      break;
    ...
    case 'error':
      console.error('[nuxc:hmr]', msg.message, msg.stack ?? '');
      break;
  }
}
...
// Expose test/debug utilities on __nuxcHmr
(window as any).__nuxcHmr = (window as any).__nuxcHmr ?? {};
(window as any).__nuxcHmr.simulate = (msg: unknown) => onMessage(JSON.stringify(msg));
