...
declare const __ZEPTR_HMR_URL__: string | undefined;

function resolveHmrUrl(): string {
  if (typeof __ZEPTR_HMR_URL__ !== 'undefined') return __ZEPTR_HMR_URL__;
  if (typeof import.meta !== 'undefined' && (import.meta as any).env?.ZEPTR_HMR_URL) {
    return (import.meta as any).env.ZEPTR_HMR_URL;
  }
  const proto = location.protocol === 'https:' ? 'wss:' : 'ws:';
  return `${proto}//${location.host}/__zeptr_hmr`;
}
...
function stamp(key: string) {
  (window as any).__zeptrHmr = (window as any).__zeptrHmr ?? {};
  (window as any).__zeptrHmr[key] = Date.now();
}
  ...
  for (const mod of modules) {
    ...
    if (cbs.length === 0) {
      ...
      try {
        ...
        console.error('[zeptr:hmr] Failed to hot-update', mod, err);
        handleFullReload();
        return;
      }
    }
...
function onMessage(raw: string): void {
  ...
  switch (msg.type) {
    case 'connected':
      console.debug('[zeptr:hmr] connected');
      reconnectDelay = 1000;
      stamp('connected');
      break;
    ...
    case 'error':
      console.error('[zeptr:hmr]', msg.message, msg.stack ?? '');
      break;
  }
}
...
// Expose test/debug utilities on __zeptrHmr
(window as any).__zeptrHmr = (window as any).__zeptrHmr ?? {};
(window as any).__zeptrHmr.simulate = (msg: unknown) => onMessage(JSON.stringify(msg));
