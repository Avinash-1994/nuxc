/**
 * @nuxco/plugin-legacy — Legacy Browser Support via SWC + core-js injection
 ...
 * In production this calls nuxco-native; in test environments it uses
...
async function downlevelWithSwc(code: string, targets: string[]): Promise<string> {
  try {
    ...
    // @ts-ignore — nuxco-native is optional in test environments
...
/**
 * Main plugin factory — call with options, register in nuxco.config.ts plugins[]
 */
export function nuxcoPluginLegacy(options: LegacyPluginOptions = {}): any {
  const {
  ...
  return {
    ...
    // Nuxco build hook: write legacy files after main bundle
...
export default nuxcoPluginLegacy;
