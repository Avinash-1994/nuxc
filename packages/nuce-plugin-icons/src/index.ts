/**
 * @nuce/plugin-icons — 100,000+ Icons, Zero Bundle Bloat
...
export interface IconsPluginOptions {
  ...
  /** Cache directory (default: .nuce/cache/icons) */
  cacheDir?: string;
}
...
export function icons(options: IconsPluginOptions = {}) {
  const {
    compiler = 'react',
    defaultClass = 'nuce-icon',
    cacheDir = '.nuce/cache/icons',
    customCollections = {},
  } = options;
  ...
  return {
    ...
    async load(id: string) {
      ...
      if (!svg) {
        console.warn(`[nuce:icons] Icon not found: ${collection}/${name}`);
