/**
 * @zeptr/plugin-legacy — Legacy Browser Support via SWC + core-js injection
 ...
 * In production this calls zeptr-native; in test environments it uses
...
async function downlevelWithSwc(code: string, targets: string[]): Promise<string> {
  try {
    ...
    // @ts-ignore — zeptr-native is optional in test environments
...
/**
 * Main plugin factory — call with options, register in zeptr.config.ts plugins[]
 */
export function zeptrPluginLegacy(options: LegacyPluginOptions = {}): any {
  const {
  ...
  return {
    ...
    // Zeptr build hook: write legacy files after main bundle
...
export default zeptrPluginLegacy;
