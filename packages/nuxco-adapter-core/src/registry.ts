// packages/nuxco-adapter-core/src/registry.ts
import type { NuxcoAdapter, PackageJson } from './index.js';

export class AdapterRegistry {
  private adapters: NuxcoAdapter[] = [];

  register(adapter: NuxcoAdapter) {
  ...
  private activeAdapter: NuxcoAdapter | null = null;

  getActiveAdapter(): NuxcoAdapter | null {
    return this.activeAdapter;
  }

  detect(projectRoot: string, pkg: PackageJson): NuxcoAdapter | null {
    ...
    for (const adapter of sortedAdapters) {
      if (adapter.detect(projectRoot, pkg)) {
        if (adapter.name === 'gatsby') {
          console.warn('[NUXCO:INFO] Gatsby adapter is currently in community preview. Report issues at https://nuxco.dev/issues');
        }
