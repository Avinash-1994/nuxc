 ...
 *
 * Interfaces with S3 or Nuce Cloud to fetch/store build artifacts.
 ...
 *  - nuce-cloud: Nuce Cloud managed cache (requires token)
 *
 * Options:
...
export class NuceCloudProvider implements RemoteCacheProvider {
  constructor(private options: RemoteCacheOptions) {}
  ...
  private getCloudUrl(key: string): string {
    const baseUrl = this.options.baseUrl || 'https://cache.nuce.dev/v1';
...
export function createRemoteCache(options: RemoteCacheOptions | boolean): RemoteCacheProvider {
  ...
  const opts = typeof options === 'boolean' ? { provider: 'nuce-cloud' as const } : options;

  switch (opts.provider) {
    case 's3':
      return new S3Provider(opts);
    case 'nuce-cloud':
      return new NuceCloudProvider(opts);
    default:
      return new NullProvider();
  }
}
