...
declare const __NUCE_HMR_URL__: string | undefined;

function resolveHmrUrl(): string {
  if (typeof __NUCE_HMR_URL__ !== 'undefined') return __NUCE_HMR_URL__;
  if (typeof import.meta !== 'undefined' && (import.meta as any).env?.NUCE_HMR_URL) {
    return (import.meta as any).env.NUCE_HMR_URL;
  }
  const proto = location.protocol === 'https:' ? 'wss:' : 'ws:';
  return `${proto}//${location.host}/__nuce_hmr`;
}
...
function stamp(key: string) {
  (window as any).__nuceHmr = (window as any).__nuceHmr ?? {};
  (window as any).__nuceHmr[key] = Date.now();
}
  ...
  for (const mod of modules) {
    ...
    if (cbs.length === 0) {
      ...
      try {
        ...
        console.error('[nuce:hmr] Failed to hot-update', mod, err);
        handleFullReload();
        return;
      }
    }
...
function onMessage(raw: string): void {
  ...
  switch (msg.type) {
    case 'connected':
      console.debug('[nuce:hmr] connected');
      reconnectDelay = 1000;
      stamp('connected');
      break;
    ...
    case 'error':
      console.error('[nuce:hmr]', msg.message, msg.stack ?? '');
      break;
  }
}
...
// Expose test/debug utilities on __nuceHmr
(window as any).__nuceHmr = (window as any).__nuceHmr ?? {};
(window as any).__nuceHmr.simulate = (msg: unknown) => onMessage(JSON.stringify(msg));
