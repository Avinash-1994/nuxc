// @zeptr/plugin-visualizer — Self-contained Unit Tests
...
describe('@zeptr/plugin-visualizer — plugin structure', () => {
  it('has correct name', () => {
    assert.equal(visualizerPlugin().name, '@zeptr/plugin-visualizer');
  });
  ...
  it('uses correct defaults', () => {
    ...
    assert.equal(p._options.title, 'Zeptr Bundle Visualizer');
...
describe('@zeptr/plugin-visualizer — load hook', () => {
  it('returns null for all IDs', () => {
    const p = visualizerPlugin();
    assert.equal(p.load('/src/App.tsx'), null);
    assert.equal(p.load('/__zeptr_entry__'), null);
    assert.equal(p.load('/node_modules/react/index.js'), null);
  });
});

describe('@zeptr/plugin-visualizer — transform hook (passthrough)', () => {
