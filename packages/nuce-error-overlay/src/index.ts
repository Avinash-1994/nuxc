 ...
 *
 * Provides a rich, browser-rendered error overlay for the Nuce dev server:
...
export function formatOverlayTitle(err: NuceError): string {
  return KIND_LABELS[err.kind] ?? 'Build Error';
}

export function formatOverlaySubtitle(err: NuceError): string {
  return `${err.location.file}:${err.location.line}${err.location.column != null ? `:${err.location.column}` : ''}`;
}

// ─── HTML overlay renderer ────────────────────────────────────────────────────

export function renderOverlayHTML(err: NuceError): string {
  ...
  const codeFrame = err.codeFrame
    ? `<pre class="nuce-code-frame">${escapeHtml(err.codeFrame)}</pre>`
    : '';
  ...
  return `
<div id="nuce-error-overlay" style="
  ...
  <div style="
    ...
    <div style="margin-top:16px;display:flex;gap:12px;flex-wrap:wrap;">
      ...
      <button onclick="document.getElementById('nuce-error-overlay').remove()" style="
...
(function() {
  if (typeof window.__nuce_hmr__ === 'undefined') return;
  window.__nuce_hmr__.on('update', function() {
    var overlay = document.getElementById('nuce-error-overlay');
    if (overlay) {
...
export function createOverlay(rawError: {
  ...
  const err: NuceError = { kind, message: rawError.message, location, codeFrame: rawError.codeFrame, stack: rawError.stack };
  return {
