// packages/nuxc-adapter-core/src/registry.ts
import type { NuxcAdapter, PackageJson } from './index.js';

export class AdapterRegistry {
  private adapters: NuxcAdapter[] = [];

  register(adapter: NuxcAdapter) {
  ...
  private activeAdapter: NuxcAdapter | null = null;

  getActiveAdapter(): NuxcAdapter | null {
    return this.activeAdapter;
  }

  detect(projectRoot: string, pkg: PackageJson): NuxcAdapter | null {
    ...
    for (const adapter of sortedAdapters) {
      if (adapter.detect(projectRoot, pkg)) {
        if (adapter.name === 'gatsby') {
          console.warn('[NUXC:INFO] Gatsby adapter is currently in community preview. Report issues at https://nuxc.dev/issues');
        }
