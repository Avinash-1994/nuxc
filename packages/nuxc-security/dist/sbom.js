 ...
 * Generates dist/nuxc-sbom.json listing every npm + Rust dep
 ...
 * @param projectRoot – root of the Nuxc project
...
/** Write the SBOM to dist/nuxc-sbom.json */
export function writeSBOM(report, distDir) {
    const outPath = path.join(distDir, 'nuxc-sbom.json');
    fs.mkdirSync(distDir, { recursive: true });
    fs.writeFileSync(outPath, JSON.stringify(report, null, 2), 'utf8');
    console.info(`[nuxc:security] SBOM written → ${outPath} (${report.components.length} components)`);
}
