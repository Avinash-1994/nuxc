...
export function generateSecurityHeaders(distDir, csp) {
    ...
    const nginx = `# Nuxc Security Headers — Nginx
...
add_header Content-Security-Policy "${csp}" always;`;
    const apache = `# Nuxc Security Headers — Apache
...
Header always set Content-Security-Policy "${csp}"`;
    ...
    const netlify = `# Nuxc Security Headers — Netlify (_headers file)
/*
${headerLines.map(([k, v]) => `  ${k}: ${v}`).join('\n')}`;
    const cloudflare = `// Nuxc Security Headers — Cloudflare Workers
...
export default {
  async fetch(request, env) {
    ...
    if (fs.existsSync(distDir)) {
        fs.writeFileSync(path.join(distDir, 'nuxc-headers.json'), JSON.stringify(result, null, 2), 'utf8');
        console.info(`[nuxc:security] Security headers → ${path.join(distDir, 'nuxc-headers.json')}`);
    }
    return result;
}
