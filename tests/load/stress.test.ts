/**
 * Load and Stress Tests for Zeptr Build System
 * 
...
describe('Load Testing: Concurrent Builds', () => {
    ...
    beforeAll(() => {
        tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeptr-load-test-'));
...
describe('Stress Testing: Large Projects', () => {
    ...
    beforeAll(() => {
        tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeptr-large-test-'));
