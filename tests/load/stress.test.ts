/**
 * Load and Stress Tests for Nuxco Build System
 * 
...
describe('Load Testing: Concurrent Builds', () => {
    ...
    beforeAll(() => {
        tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nuxco-load-test-'));
...
describe('Stress Testing: Large Projects', () => {
    ...
    beforeAll(() => {
        tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nuxco-large-test-'));
