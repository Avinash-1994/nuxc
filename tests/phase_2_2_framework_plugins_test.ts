...
import { lunxReact } from '../src/plugins/frameworks/react.js';
import { lunxVue } from '../src/plugins/frameworks/vue.js';
import { strict as assert } from 'assert';

async function testReactBasicTransform() {
    console.log('\n[Test 1] React - Basic JSX Transform');

    const plugin = lunxReact({ development: true, fastRefresh: false });
...
async function testReactCSSTracking() {
    ...
    const plugin = lunxReact({ development: true });
...
async function testReactComponentDetection() {
    ...
    const plugin = lunxReact({ development: true, fastRefresh: true });
...
async function testVueBasicSFC() {
    ...
    const plugin = lunxVue({ development: true });
...
async function testReactProductionMode() {
    ...
    const plugin = lunxReact({ development: false, fastRefresh: false });
...
async function testVueProductionMode() {
    ...
    const plugin = lunxVue({ development: false, hmr: false });
