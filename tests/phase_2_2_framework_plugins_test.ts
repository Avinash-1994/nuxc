...
import { zeptrReact } from '../src/plugins/frameworks/react.js';
import { zeptrVue } from '../src/plugins/frameworks/vue.js';
import { strict as assert } from 'assert';

async function testReactBasicTransform() {
    console.log('\n[Test 1] React - Basic JSX Transform');

    const plugin = zeptrReact({ development: true, fastRefresh: false });
...
async function testReactCSSTracking() {
    ...
    const plugin = zeptrReact({ development: true });
...
async function testReactComponentDetection() {
    ...
    const plugin = zeptrReact({ development: true, fastRefresh: true });
...
async function testVueBasicSFC() {
    ...
    const plugin = zeptrVue({ development: true });
...
async function testReactProductionMode() {
    ...
    const plugin = zeptrReact({ development: false, fastRefresh: false });
...
async function testVueProductionMode() {
    ...
    const plugin = zeptrVue({ development: false, hmr: false });
