...
async function loadConfig(cwd) {
  ...
  const ymlPath = path.join(cwd, "zeptr.build.yml");
  const tsPath = path.join(cwd, "zeptr.build.ts");
  let rawConfig;
  try {
    if (await fs.access(jsonPath).then(() => true).catch(() => false)) {
      ...
      const outfile = path.join(cwd, "zeptr.build.temp.mjs");
      await build({
        entryPoints: [tsPath],
        outfile,
        bundle: true,
...
async function testYamlConfig() {
  ...
  const yamlPath = path2.join(cwd, "zeptr.build.yaml");
  const yamlContent = `
root: .
entry: 
  - src/main.tsx
