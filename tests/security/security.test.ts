...
import { scanSecrets } from '../../packages/nuxc-security/src/secret-scanner.js';
import { computeSRI } from '../../packages/nuxc-security/src/sri.js';
import { generateCSP } from '../../packages/nuxc-security/src/csp.js';
import { auditLockfile } from '../../packages/nuxc-security/src/lockfile-audit.js';
import { validatePath, validateSymlink, guardEnvAccess } from '../../packages/nuxc-security/src/build-isolation.js';
import { generateSBOM } from '../../packages/nuxc-security/src/sbom.js';
import { scanCVE } from '../../packages/nuxc-security/src/cve-scan.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const PROJECT_ROOT = path.resolve(__dirname, '../../');
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), 'nuxc-sec-'));
...
describe('SEC-TEST-015: SBOM contains all direct deps', () => {
  it('generateSBOM returns CycloneDX format', async () => {
    ...
    expect(sbom.metadata.tools[0].name).toBe('nuxc');
  });
});
