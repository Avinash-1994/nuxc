...
async function runOverlayTest() {
    ...
    // Verify UI
    const overlay = document.querySelector('zeptr-error-overlay');
    ...
    (window as any).__ZEPTR_OVERLAY__.reportBuildError({
    ...
    for (let i = 0; i < 100; i++) {
        (window as any).__ZEPTR_OVERLAY__.reportBuildError({ message: `Err ${i}` });
