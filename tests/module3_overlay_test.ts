...
async function runOverlayTest() {
    ...
    // Verify UI
    const overlay = document.querySelector('nuce-error-overlay');
    ...
    (window as any).__NUCE_OVERLAY__.reportBuildError({
    ...
    for (let i = 0; i < 100; i++) {
        (window as any).__NUCE_OVERLAY__.reportBuildError({ message: `Err ${i}` });
