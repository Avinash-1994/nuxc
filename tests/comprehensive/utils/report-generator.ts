...
export class ReportGenerator {
    ...
    private analyzeGaps(comparisons: ComparisonData[]): GapAnalysis[] {
        ...
        for (const comp of comparisons) {
            const nuceValue = comp.tools.nuce;
            if (nuceValue === null) continue;

            const winnerValue = comp.tools[comp.winner];
            if (winnerValue === null || comp.winner === 'nuce') continue;

            const diff = ((nuceValue - winnerValue) / winnerValue) * 100;

            if (diff > 50) {
                perfGaps.push({
                    issue: `${comp.metric}: Nuce is ${diff.toFixed(0)}% slower than ${comp.winner}`,
    ...
    private generateMarkdownReport(
        ...
        let md = `# 🧪 Nuce Comprehensive Test Report\n\n`;
        ...
        const wins = comparisons.filter(c => c.winner === 'nuce').length;
        ...
        md += `| Metric | Nuce | Vite | Webpack | Rspack | esbuild | Turbopack | Parcel | Winner |\n`;
        md += `|--------|-------|------|---------|--------|---------|-----------|--------|--------|\n`;

        for (const comp of comparisons) {
            const row = [
                comp.metric,
                this.formatValue(comp.tools.nuce, comp.unit, comp.winner === 'nuce'),
        ...
        const winningMetrics = comparisons.filter(c => c.winner === 'nuce');
        if (winningMetrics.length > 0) {
            for (const metric of winningMetrics) {
                const secondBest = this.getSecondBest(metric);
                const improvement = secondBest ? this.calculateImprovement(metric.tools.nuce!, secondBest.value) : 0;
                md += `- **${metric.metric}**: ${metric.tools.nuce}${metric.unit}`;
                if (secondBest) {
            ...
            md += `*No metrics where Nuce is currently winning*\n`;
        }
        md += `\n`;

        // Where We're Lagging
        md += `## ⚠️  Where We're Lagging\n\n`;
        const laggingMetrics = comparisons.filter(c => c.winner !== 'nuce' && c.tools.nuce !== null);
        if (laggingMetrics.length > 0) {
            for (const metric of laggingMetrics) {
                const winnerValue = metric.tools[metric.winner]!;
                const nuceValue = metric.tools.nuce!;
                const gap = this.calculateGap(nuceValue, winnerValue);
                md += `- **${metric.metric}**: ${nuceValue}${metric.unit} vs ${metric.winner} ${winnerValue}${metric.unit} (${gap.toFixed(0)}% slower)\n`;
            }
        } else {
            md += `*Nuce is winning or competitive in all tested metrics!*\n`;
        }
        md += `\n`;
...
<head>
  ...
  <title>Nuce Comprehensive Test Report</title>
  <style>
...
<body>
  <div class="container">
    <header>
      <h1>🧪 Nuce Comprehensive Test Report</h1>
          ...
          <tbody>
            ${comparisons.map(comp => `
              <tr>
                ...
                <td class="${comp.winner === 'nuce' ? 'winner' : ''}">${this.formatValue(comp.tools.nuce, comp.unit, comp.winner === 'nuce')}</td>
    ...
    private getAllTools(): BuildTool[] {
        return ['nuce', 'vite', 'webpack', 'rspack', 'esbuild', 'turbopack', 'parcel'];
    }
    ...
    private getSecondBest(comp: ComparisonData): { tool: BuildTool; value: number } | null {
        const values = Object.entries(comp.tools)
            .filter(([tool, value]) => tool !== 'nuce' && value !== null)
    ...
    private calculateImprovement(nuceValue: number, otherValue: number): number {
        return ((otherValue - nuceValue) / otherValue) * 100;
    }

    private calculateGap(nuceValue: number, winnerValue: number): number {
        return ((nuceValue - winnerValue) / winnerValue) * 100;
    }
}
