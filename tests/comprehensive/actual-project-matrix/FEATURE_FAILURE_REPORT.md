# 🔍 Nuxco Feature Failure Report

This report provides a deep dive into specifically which Nuxco features are failing and why, based on real-project edge cases.
...
*   **Symptom**: Nuxco fails immediately if `nuxco.config.js` slightly deviates from the latest internal Zod schema.
*   **Issue**: Most build tools (like Vite) auto-convert strings to arrays for convenience. Nuxco is currently too strict.
...
*   **Hypothesis**: Nuxco might not be correctly identifying the "sideEffects" flag in Alpine's vanilla structure, leading to over-aggressive or under-aggressive shaking.
