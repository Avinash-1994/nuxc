...
var MutationCache = (_a7 = class extends Subscribable {
  ...
  remove(mutation) {
    ...
    if (__privateGet(this, _mutations).has(scope)) {
      const mutations = __privateGet(this, _mutations).get(scope)?.filter((x) => x !== mutation);
      if (mutations) {
  ...
  canRun(mutation) {
    const firstPendingMutation = __privateGet(this, _mutations).get(scopeFor(mutation))?.find((m) => m.state.status === "pending");
    return !firstPendingMutation || firstPendingMutation === mutation;
  }
  runNext(mutation) {
    const foundMutation = __privateGet(this, _mutations).get(scopeFor(mutation))?.find((m) => m !== mutation && m.state.isPaused);
...
//# sourceMappingURL=_tanstack_react-query.js.map
