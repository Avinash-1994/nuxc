...
const PROJECTS = [
  { id: 1, name: 'Nuxco Core', tasks: 12, done: 8 },
...
function Nav() {
  ...
  return (
    <nav style={{background:'#0f172a',color:'#fff',padding:'0 24px',display:'flex',gap:24,alignItems:'center',height:56}}>
      <span style={{fontWeight:700,fontSize:18,marginRight:16}}>⚡ Nuxco Tasks</span>
      {links.map(([to, label]) => (
  ...
  return (
    <div style={{padding:32,maxWidth:400}}>
      ...
      <form onSubmit={e=>{e.preventDefault();setOk(true)}} style={{display:'flex',flexDirection:'column',gap:16}}>
        <input type="email" placeholder="Email" defaultValue="dev@nuxco.dev" required style={{padding:12,borderRadius:8,border:'1px solid #334155',background:'#1e293b',color:'#fff'}}/>
