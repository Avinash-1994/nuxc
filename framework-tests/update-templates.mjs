...
.nuxco-logo{display:inline-flex;align-items:center;gap:10px;background:rgba(99,102,241,.12);border:1px solid rgba(99,102,241,.3);border-radius:999px;padding:6px 18px;font-size:13px;font-weight:600;color:var(--accent);margin-bottom:32px;backdrop-filter:blur(10px)}
.nuxco-logo::before{content:'⚡';font-size:16px}
h1{font-size:clamp(42px,7vw,80px);font-weight:900;line-height:1.05;margin-bottom:10px;letter-spacing:-2px}
h1 .nuxco{background:linear-gradient(135deg,#e0e7ff,#818cf8,#6366f1);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
...
<title>${fw.name} + Nuxco</title>
<style>${CSS}</style>
</head>
<body>
${mountDiv}
<header>
  <div class="nuxco-logo">Nuxco Build Tool</div>
  <h1><span class="nuxco">Welcome to Nuxco</span><br/>
  ...
  <div class="meta-row">
    ...
    <div class="meta-chip"><strong>🏗️ Nuxco Engine</strong>Build Tool</div>
...
<main>
  ...
  <div class="cards">
    <div class="card hero">
      ...
      <div class="hero-text">
        ...
        <p>Created with <strong>Nuxco</strong> — the high-performance build tool powered by Rust and esbuild. Edit <code style="color:#818cf8;background:#6366f115;padding:2px 8px;border-radius:6px;font-size:13px">src/App.${ext}</code> to start building your app.</p>
      </div>
    </div>
    ${coreCards}
    <div class="card">
  ...
  <div class="steps">
    <div class="step"><div class="sn">1</div><div><h4>Create project</h4><code>nuxco create my-app --framework ${fwSlug}</code></div></div>
    <div class="step"><div class="sn">2</div><div><h4>Start dev server</h4><code>cd my-app && nuxco dev</code></div></div>
    <div class="step"><div class="sn">3</div><div><h4>Edit your app</h4><code>src/App.${ext}</code></div></div>
    <div class="step"><div class="sn">4</div><div><h4>Build for prod</h4><code>nuxco build --optimize</code></div></div>
  </div>

  <div class="section-head"><span>Nuxco CLI</span></div>
  <div class="term">
    <div class="th">
    ...
    <div class="ln"><span class="cmt"># Create a ${fw.name} project with Nuxco</span></div>
    <div class="ln"><span class="prompt">$ </span><span class="cmd">nuxco create my-${fwSlug}-app --framework ${fwSlug}</span></div>
    <div class="ln"><span class="ok">  ✓ Scaffolded ${fw.name} project in 0.3s</span></div>
    <div class="ln"><span class="prompt">$ </span><span class="cmd">nuxco dev</span></div>
    <div class="ln"><span class="ok">  ✓ Compiled successfully!</span> <span class="dim">→ http://localhost:5173</span></div>
    <div class="ln"><span class="prompt">$ </span><span class="cmd">nuxco build --optimize</span></div>
    <div class="ln"><span class="ok">  ✓ Build complete in 1.2s — 42 modules bundled</span></div>
  </div>
</main>

<footer>
  <div class="footer-glow">⚡ Powered by Nuxco</div>
  <p>Built with <a href="#">Nuxco Build Tool</a> · Framework: <strong style="color:${fw.color}">${fw.name} v${fw.v}</strong> · <a href="#">Documentation</a> · <a href="#">GitHub</a></p>
</footer>
${scriptTag}
</body>
</html>`;
