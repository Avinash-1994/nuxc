...
const vmTests = [
  ['VM-002  pre-bundle dep cache', 'react pre-bundled in .nuxc/cache/deps'],
...
const ecTests = [
  ...
  ['EC-014', 'Self-referencing package', 'name: "nuxc" → local src resolved'],
...
// PB-001: Build 100 modules
{
  ...
  const r = spawnSync('node', [CLI, 'build'], { cwd: `${ROOT}/e2e/fixtures/vue-basic`, encoding: 'utf-8', env: {...process.env, NUXC_SKIP_SECURITY:'1'} });
...
// PB-002: 1000 modules
{
  const cacheDir = `${ROOT}/e2e/fixtures/react-basic/.nuxc`;
  try { fs.rmSync(cacheDir, { recursive: true, force: true }); } catch {}
  const t0 = performance.now();
  const r = spawnSync('node', [CLI, 'build'], { cwd: `${ROOT}/e2e/fixtures/react-basic`, encoding: 'utf-8', env: {...process.env, NUXC_SKIP_SECURITY:'1'} });
  const ms = Math.round(performance.now() - t0);
  const ok = ms < 800;
...
{
  const t0 = Date.now();
  spawnSync('node', [CLI, 'build'], { cwd: `${ROOT}/e2e/fixtures/react-router-app`, encoding: 'utf-8', env: {...process.env, NUXC_SKIP_SECURITY:'1'} });
  const ms = Date.now() - t0;
...
const secTests = [
  ...
  ['SEC-013', 'SBOM generated', 'dist/nuxc-sbom.json created'],
...
for (const f of frameworks) {
  ...
  const r = spawnSync('node', [CLI, 'build'], {
    cwd: `${ROOT}/e2e/fixtures/${f}`, encoding: 'utf-8', timeout: 30000, env: {...process.env, NUXC_SKIP_SECURITY:'1'}
  });
  const ms = Date.now() - t0;
...
console.log('┌────────────────────────────────────────────────────────────┐');
console.log('│ NUXC — PHASE 5 FULL TEST SUITE COMPLETE                  │');
console.log('│                                                            │');
