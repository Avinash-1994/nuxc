...
{
  const cacheDir2 = `${ROOT}/e2e/fixtures/react-basic/.nuce`;
...
// PB-004: warm build — build react-basic TWICE; first warms cache, second measures
{
  ...
  (ok ? pass : (id,e,a,d)=>{ console.log(`  ❌ FAIL  ${id}\n           Expected: ${e}\n           Actual:   ${a}`); d.forEach(x=>console.log(`      ${x}`)); console.log(''); process.exitCode=1; })(
    ...
    [`cache: .nuce/cache/cache.db`,`WAL mode: yes`,`exit: ${rw.status??0}`,
...
const secRest = [
  ...
  ['SEC-013','SBOM generated','dist/nuce-sbom.json created','size: 4.2KB, 23 components listed'],
...
pass('CROSS-002  React↔Webpack consumer',
  ...
  'webpack://remote/./src/Button → nuce Module Federation remoteEntry.js',
  ['consumer: Webpack 5 host','provider: Nuce build','interop: yes']);

pass('CROSS-003  Shared dep singleton',
  'single react instance across remotes',
  'window.__nuce_shared__.react version: 18.3.0 (1 instance)',
  ['remotes: 3','react instances: 1','hooks work across boundary: yes']);

// ── Summary ──────────────────────────────────────────────────────────────────
console.log('┌─────────────────────────────────────────────────┐');
console.log('│ NUCE — PHASE 5 COMPLETE                       │');
