...
async function runTest() {
  ...
  const customCacheDir = path.join('/tmp', 'nuxco-test-cache');
  const defaultCacheDir = path.join(rootDir, '.nuxco', 'cache');
  ...
  const cacheManager = getCacheManager(rootDir, { cacheDir: '/tmp/nuxco-test-cache' });
  ...
  printPass('PB-06-REG  Custom cacheDir is respected', 'cache files exist at /tmp/nuxco-test-cache/', 'cache files exist at /tmp/nuxco-test-cache/');
