...
async function runTest() {
  ...
  const customCacheDir = path.join('/tmp', 'zeptr-test-cache');
  const defaultCacheDir = path.join(rootDir, '.zeptr', 'cache');
  ...
  const cacheManager = getCacheManager(rootDir, { cacheDir: '/tmp/zeptr-test-cache' });
  ...
  printPass('PB-06-REG  Custom cacheDir is respected', 'cache files exist at /tmp/zeptr-test-cache/', 'cache files exist at /tmp/zeptr-test-cache/');
