...
async function runTest() {
  ...
  const customCacheDir = path.join('/tmp', 'nuxc-test-cache');
  const defaultCacheDir = path.join(rootDir, '.nuxc', 'cache');
  ...
  const cacheManager = getCacheManager(rootDir, { cacheDir: '/tmp/nuxc-test-cache' });
  ...
  printPass('PB-06-REG  Custom cacheDir is respected', 'cache files exist at /tmp/nuxc-test-cache/', 'cache files exist at /tmp/nuxc-test-cache/');
