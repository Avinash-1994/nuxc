...
await (async function testStreaming() {
  ...
  await new Promise((resolve, reject) => {
    const stream = entryServer.renderToStream({
      url: '/dashboard',
      cookies: { session: 'nuxco-session-abc123' }
    });
...
await (async function testColdStartHmrBuild() {
  ...
  const serverReady = new Promise((resolve) => {
    const onData = (chunk) => {
      ...
      serverOutput += text;
      // Match the Nuxco startup banner
      if (
  ...
  const hmrChange = hmrOriginal + `\n// nuxco-hmr-ts-${Date.now()}: exported const updated`;
  ...
  (buildPass ? printPass : printFail)(
    ...
    [
      ...
      !hasRealJs
        ? 'Reason: build did not emit compiled JS — nuxco build for SolidStart must produce compiled JavaScript bundles'
        : 'OK',
    ]
  );
})();
