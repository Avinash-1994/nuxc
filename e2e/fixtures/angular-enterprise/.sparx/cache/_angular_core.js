    ...
    return (e) => {
      zone.runOutsideAngular(() => {
        if (injector.destroyed && !userErrorHandler) {
          ...
          userErrorHandler ?? (userErrorHandler = injector.get(ErrorHandler));
...
function createViewEffect(view, notifier, fn) {
  ...
  view[EFFECTS] ?? (view[EFFECTS] = /* @__PURE__ */ new Set());
...
var _DehydratedBlockRegistry = class _DehydratedBlockRegistry {
  constructor() {
    ...
    __publicField(this, "jsActionMap", inject2(JSACTION_BLOCK_ELEMENT_MAP));
    __publicField(this, "contract", inject2(JSACTION_EVENT_CONTRACT));
...
__publicField(_DehydratedBlockRegistry, "\u0275prov", /* @__PURE__ */ \u0275\u0275defineInjectable({
...
function getLViewEnterAnimations(lView) {
  var _a4;
  const animationData = lView[_a4 = ANIMATIONS] ?? (lView[_a4] = {});
...
var _AfterRenderImpl = class _AfterRenderImpl {
  ...
  register(sequence) {
    ...
    if (view !== void 0) {
      (view[_a4 = AFTER_RENDER_SEQUENCES_TO_ADD] ?? (view[_a4] = [])).push(sequence);
...
var ViewRef = class {
  ...
  checkNoChanges() {
    if (ngDevMode) {
      try {
        this.exhaustive ?? (this.exhaustive = this._lView[INJECTOR].get(UseExhaustiveCheckNoChanges, USE_EXHAUSTIVE_CHECK_NO_CHANGES_DEFAULT));
      } catch {
...
}, __publicField(_a3, "NULL", new _NullComponentFactoryResolver()), _a3);
...
function setupSelectorMatchedInputsOrOutputs(mode, tNode, def, directiveIndex) {
  ...
  for (const publicName in aliasMap) {
    if (aliasMap.hasOwnProperty(publicName)) {
      let bindings;
      if (mode === 0) {
        bindings = tNode.inputs ?? (tNode.inputs = {});
      } else {
        bindings = tNode.outputs ?? (tNode.outputs = {});
      }
      bindings[publicName] ?? (bindings[publicName] = []);
...
function setupHostDirectiveInputsOrOutputs(mode, tNode, config, directiveIndex) {
  ...
  for (const initialName in aliasMap) {
    if (aliasMap.hasOwnProperty(initialName)) {
      ...
      let bindings;
      if (mode === 0) {
        bindings = tNode.hostDirectiveInputs ?? (tNode.hostDirectiveInputs = {});
      } else {
        bindings = tNode.hostDirectiveOutputs ?? (tNode.hostDirectiveOutputs = {});
      }
      bindings[publicName] ?? (bindings[publicName] = []);
...
function createLQuery(tView, lView, flags) {
  ...
  const lQueries = (lView[_a4 = QUERIES] ?? (lView[_a4] = new LQueries_())).queries;
...
function resolveHostDirectives(matches) {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    if (def.hostDirectives !== null) {
      ...
      hostDirectiveRanges ?? (hostDirectiveRanges = /* @__PURE__ */ new Map());
...
function trackTriggerForDebugging(tView, tNode, textRepresentation) {
  ...
  (_a4 = tDetails.debug).triggers ?? (_a4.triggers = /* @__PURE__ */ new Set());
...
function handleEffectCreatedEvent(context, effect2) {
  ...
  cleanupContainer.onDestroyFns ?? (cleanupContainer.onDestroyFns = []);
...
var _ApplicationInitStatus = class _ApplicationInitStatus {
  constructor() {
    ...
    __publicField(this, "appInits", inject2(APP_INITIALIZER, {
      optional: true
    }) ?? []);
...
function runLeaveAnimations(lView, tNode, value, ngZone) {
  ...
  ((_a4 = getLViewLeaveAnimations(lView).get(tNode.index)).resolvers ?? (_a4.resolvers = [])).push(resolve);
  const activeClasses = getClassListFromValue(value);
  if (activeClasses && activeClasses.length > 0) {
...
function listenerInternal(tView, lView, renderer, tNode, eventName, listenerFn, eventTargetResolver) {
  ...
  if (tNode.type & 3 || eventTargetResolver) {
    wrappedListener ?? (wrappedListener = wrapListener(tNode, lView, listenerFn));
...
var _ChangeDetectionSchedulerImpl = class _ChangeDetectionSchedulerImpl {
  constructor() {
    ...
    __publicField(this, "angularZoneId", this.zoneIsDefined ? this.ngZone._inner?.get(angularZoneInstanceIdProperty) : null);
    __publicField(this, "scheduleInRootZone", !this.zonelessEnabled && this.zoneIsDefined && (inject2(SCHEDULE_IN_ROOT_ZONE, {
      optional: true
    }) ?? false));
...
var BaseWritableResource = class {
  ...
  get snapshot() {
    return this._snapshot ?? (this._snapshot = computed(() => {
...
function internalProvideZoneChangeDetection({
  ...
  ngZoneFactory ?? (ngZoneFactory = () => new NgZone({
    ...getNgZoneOptions(),
    scheduleInRootZone
  }));
  return [{
...
var DebugTaskTrackerImpl = class {
  constructor() {
    __publicField(this, "openTasks", /* @__PURE__ */ new Map());
  }
  add(taskId) {
...
function serializeLContainer(lContainer, tNode, lView, parentDeferBlockId, context) {
  ...
  for (let i = CONTAINER_HEADER_OFFSET; i < lContainer.length; i++) {
    ...
    if (views.length > 0 && currentViewAsString === lastViewAsString) {
      ...
      previousView[_a4 = MULTIPLIER] ?? (previousView[_a4] = 1);
...
function appendSerializedNodePath(ngh, tNode, lView, excludedParentNodes) {
  var _a4, _b;
  const noOffsetIndex = tNode.index - HEADER_OFFSET;
  ngh[_a4 = NODES] ?? (ngh[_a4] = {});
  (_b = ngh[NODES])[noOffsetIndex] ?? (_b[noOffsetIndex] = calcPathForNode(tNode, lView, excludedParentNodes));
}
function appendDisconnectedNodeIndex(ngh, tNodeOrNoOffsetIndex) {
  ...
  ngh[_a4 = DISCONNECTED_NODES] ?? (ngh[_a4] = []);
...
function serializeLView(lView, parentDeferBlockId = null, context) {
  ...
  for (let i = HEADER_OFFSET; i < tView.bindingStartIndex; i++) {
    ...
    if (isLContainer(lView[i])) {
      ...
      if (tNode.type & 8) {
        ngh[_d = ELEMENT_CONTAINERS] ?? (ngh[_d] = {});
...
var AfterRenderEffectSequence = class extends AfterRenderSequence {
  constructor(impl, effectHooks, view, scheduler, injector, snapshot = null) {
    ...
    __publicField(this, "onDestroyFns", null);
    ...
    for (const phase of AFTER_RENDER_PHASES) {
      ...
      node.registerCleanupFn = (fn) => (node.cleanup ?? (node.cleanup = /* @__PURE__ */ new Set())).add(fn);
...
var SnapshotResource = class {
  constructor(snapshot) {
    __publicField(this, "snapshot");
    __publicField(this, "value", computed(() => {
      if (this.state.status === "error") {
        throw new ResourceValueError(this.state.error);
      }
      return this.state.value;
    }));
    __publicField(this, "status", computed(() => this.state.status));
    __publicField(this, "error", computed(() => this.state.status === "error" ? this.state.error : void 0));
    __publicField(this, "isLoading", computed(() => this.state.status === "loading" || this.state.status === "reloading"));
    __publicField(this, "isValueDefined", computed(() => this.state.status !== "error" && this.state.value !== void 0));
    this.snapshot = snapshot;
  }
  get state() {
...
export {
  ...
  NgModuleFactory2 as \u0275NgModuleFactory,
  ...
  ComponentFactory2 as \u0275Render3ComponentFactory,
  ComponentRef2 as \u0275Render3ComponentRef,
  NgModuleRef2 as \u0275Render3NgModuleRef,
