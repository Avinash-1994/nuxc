 ...
 *
 * Two layers:
 *  A) Client-side message handling — driven with window.__nuxcHmr.simulate()
...
async function run() {
    ...
    try {
        ...
        await page.waitForFunction(() => !!window.__nuxcHmr?.connected, { timeout: 10_000 });
        ...
        // A1 — update message stamps lastUpdate
        {
            ...
            await page.evaluate(() =>
                window.__nuxcHmr.simulate({ type: 'update', modules: ['/src/component.js'] })
            );
            await page.waitForFunction(since => (window.__nuxcHmr?.lastUpdate ?? 0) > since, before, { timeout: 5000 });
        ...
        // A2 — css-update message stamps lastCssUpdate
        {
            const before = Date.now();
            await page.evaluate(() =>
                window.__nuxcHmr.simulate({ type: 'css-update', href: '/src/styles.css' })
            );
            await page.waitForFunction(since => (window.__nuxcHmr?.lastCssUpdate ?? 0) > since, before, { timeout: 5000 });
            const delta = Date.now() - before;
        ...
        // A4 — error message logged, no reload
        {
            ...
            await page.evaluate(() =>
                window.__nuxcHmr.simulate({ type: 'error', message: 'test-err', stack: 'at test' })
            );
            await wait(300);
        ...
        // B1 — JS file change → server broadcasts update → lastUpdate stamped
        {
            ...
            await page.waitForFunction(since => (window.__nuxcHmr?.lastUpdate ?? 0) > since, before, { timeout: 8000 });
            const delta = Date.now() - before;
        ...
        // B2 — CSS file change → server broadcasts css-update → lastCssUpdate stamped
        {
            ...
            await page.waitForFunction(since => (window.__nuxcHmr?.lastCssUpdate ?? 0) > since, before, { timeout: 8000 });
            const delta = Date.now() - before;
