...
async function runTests() {
  ...
  try {
    ...
    pass('VP-04  Cold start', '< 300ms', `${Math.round(startupTime)}ms`, [
      ...
      `[lunx] adapter: vitepress in output: yes`,
      `uWS bound: yes`
    ]);
  ...
  const hasMockComments = clientContent.includes('mock waku bundle') || clientContent.includes('mock vitepress bundle') || clientContent.includes('// [Lunx]');
  ...
  log(`┌─────────────────────────────────────────────┐`);
  log(`│ LUNX — PHASE 2.12 VITEPRESS COMPLETE       │`);
