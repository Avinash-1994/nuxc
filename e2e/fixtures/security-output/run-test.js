...
// ──────────────────────────────────────────────────────────
{
  ...
  fs.writeFileSync(path.join(__dirname, 'nuxc.config.ts'), `export default { entry: ['src/index.js'] };`);
...
`);
  ...
  if (res.status !== 0) {
    ...
    const sriManifestPath = path.join(outDir, 'nuxc-sri-manifest.json');
    const cspPath = path.join(outDir, 'nuxc-csp.txt');
