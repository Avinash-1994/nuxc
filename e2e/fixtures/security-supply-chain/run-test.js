...
// ──────────────────────────────────────────────────────────
{
  ...
  const auditDbDir = path.join(__dirname, '.nuxc', 'security');
...
// ──────────────────────────────────────────────────────────
{
  ...
  fs.rmSync(path.join(__dirname, '.nuxc'), { recursive: true, force: true });
...
// ──────────────────────────────────────────────────────────
{
  ...
  fs.writeFileSync(path.join(__dirname, 'nuxc.config.json'), JSON.stringify({ entry: ['src/index.js'] }));
  fs.mkdirSync(path.join(__dirname, 'src'), { recursive: true });
  fs.writeFileSync(path.join(__dirname, 'src', 'index.js'), 'import "axios"; import "lodash"; import "date-fns"; console.log("hello");');

  const res = spawnSync('node', [cliPath, 'build'], { cwd: __dirname, encoding: 'utf8' });
  const sbomPath = path.join(__dirname, 'build_output', 'nuxc-sbom.json');
  ...
  if (generated) {
    ...
    (ok ? pass : fail)('SEC-03  SBOM Generation', `> 10 components, CycloneDX 1.5`, `${count} components`, [
      ...
      `Output file:          build_output/nuxc-sbom.json`,
    ]);
  } else {
    fail('SEC-03  SBOM Generation', 'nuxc-sbom.json generated', 'file missing or build failed', [
      `Exit code: ${res.status}`,
