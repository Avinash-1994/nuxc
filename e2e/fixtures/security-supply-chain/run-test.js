...
// ──────────────────────────────────────────────────────────
{
  ...
  const auditDbDir = path.join(__dirname, '.nuce', 'security');
...
// ──────────────────────────────────────────────────────────
{
  ...
  fs.rmSync(path.join(__dirname, '.nuce'), { recursive: true, force: true });
...
// ──────────────────────────────────────────────────────────
{
  ...
  fs.writeFileSync(path.join(__dirname, 'nuce.config.json'), JSON.stringify({ entry: ['src/index.js'] }));
  fs.mkdirSync(path.join(__dirname, 'src'), { recursive: true });
  fs.writeFileSync(path.join(__dirname, 'src', 'index.js'), 'import "axios"; import "lodash"; import "date-fns"; console.log("hello");');

  const res = spawnSync('node', [cliPath, 'build'], { cwd: __dirname, encoding: 'utf8' });
  const sbomPath = path.join(__dirname, 'build_output', 'nuce-sbom.json');
  ...
  if (generated) {
    ...
    (ok ? pass : fail)('SEC-03  SBOM Generation', `> 10 components, CycloneDX 1.5`, `${count} components`, [
      ...
      `Output file:          build_output/nuce-sbom.json`,
    ]);
  } else {
    fail('SEC-03  SBOM Generation', 'nuce-sbom.json generated', 'file missing or build failed', [
      `Exit code: ${res.status}`,
