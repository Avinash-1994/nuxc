...
// ──────────────────────────────────────────────────────────
{
  ...
  const auditDbDir = path.join(__dirname, '.sparx', 'security');
...
// ──────────────────────────────────────────────────────────
{
  ...
  fs.rmSync(path.join(__dirname, '.sparx'), { recursive: true, force: true });
...
// ──────────────────────────────────────────────────────────
{
  ...
  fs.writeFileSync(path.join(__dirname, 'sparx.config.json'), JSON.stringify({ entry: ['src/index.js'] }));
  fs.mkdirSync(path.join(__dirname, 'src'), { recursive: true });
  ...
  if (generated) {
    ...
    (ok ? pass : fail)('SEC-03  SBOM Generation', `> 10 components, CycloneDX 1.5`, `${count} components`, [
      ...
      `Output file:          build_output/sparx-sbom.json`,
    ]);
  } else {
    fail('SEC-03  SBOM Generation', 'sparx-sbom.json generated', 'file missing or build failed', [
      `Exit code: ${res.status}`,
