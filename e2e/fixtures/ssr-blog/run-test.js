 ...
 */
import { SsrRunner } from '@nuce/ssr';
...
async function run() {
    ...
    // ── Build @nuce/ssr ──────────────────────────────────────────
    log('Building @nuce/ssr package...');
    execSync('npm run build', {
        cwd: path.resolve(__dirname, '../../../packages/nuce-ssr'),
        stdio: 'ignore'
    });
    log('Build complete.\n');
...
exports.render = async function render(context) {
    ...
    const posts = Array.from({ length: 50 }, function(_, i) {
        return {
            ...
            excerpt: 'A deep dive into how Nuce resolves modules, splits chunks, and eliminates dead code. Post ' + (i + 1) + ' of the series.'
        };
    });
    ...
    const html =
        '<div id="app">' +
        '<header><h1>Nuce SSR Web Blog</h1></header>' +
    ...
    const head =
        '<title>Nuce SSR Blog — ' + url + '</title>' +
        '<meta name="description" content="A comprehensive blog generated server-side by Nuce SSR.">' +
        '<meta property="og:title" content="Nuce SSR Blog">' +
        '<link rel="canonical" href="https://blog.nuce.dev' + url + '">';
...
`;
    ...
    const runnerSrc = fs.readFileSync(
        path.resolve(__dirname, '../../../packages/nuce-ssr/src/runner.ts'), 'utf-8'
    );
    ...
    const result2 = await runner.renderToString(bundlePath, {
        url: '/home',
        headers: { 'user-agent': 'NuceTestBot/1.0' }
    });
    if (result2.error) {
        fail('T4', 'context.headers respected', `Error: ${result2.error.message}`, 'renderToString threw');
    } else if (result2.html.includes('NuceTestBot/1.0')) {
        pass('T4', 'context.headers respected',
            'HTML contains user-agent "NuceTestBot/1.0" from context.headers');
    } else {
    ...
    if (!hasVue && !hasReact) {
        pass('T14', 'Framework-agnostic: runner has no Vue/React imports',
            'No "from \'vue\'" or "from \'react\'" found in packages/nuce-ssr/src/runner.ts');
    } else {
    ...
    for (const fix of fixtures) {
        ...
        try {
            const t0 = process.hrtime.bigint();
            execSync(`node ${cliPath} build`, { cwd: fixDir, stdio: 'ignore', env: { ...process.env, NUCE_SKIP_SECURITY: '1' } });
