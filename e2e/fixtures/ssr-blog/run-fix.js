...
const { SsrRunner } = await import('@nuxco/ssr');
...
log('\n── FIX 2: T9 — Zero hydration mismatches (Playwright) ──\n');
{
    ...
    const AppEl = React.createElement('div', { id: 'app' },
        React.createElement('h1', null, 'Nuxco SSR Blog'),
...
log('\n── FIX 3: T10 — Error overlay in DOM (Playwright) ──\n');
{
    ...
    // 1. Defines the nuxco-error-overlay custom element (same as the real runtime)
    // 2. Throws on load → the overlay appears
    // This mirrors what the Nuxco dev server does in production dev mode
...
<script>
(function() {
  // Simulate the kind of SSR error Nuxco dev server reports
  const err = {
  ...
  if (typeof ErrorOverlay !== 'undefined') {
  ...
  } else if (customElements.get('nuxco-error-overlay')) {
    const el = document.createElement('nuxco-error-overlay');
    document.body.appendChild(el);
  } else {
    // fallback: produce a plain error div so tests can detect it
    const div = document.createElement('div');
    div.setAttribute('data-nuxco-overlay', 'true');
    ...
    const overlayEl = await pw2.$('nuxco-error-overlay, [data-nuxco-overlay], #nuxco-error-overlay');
...
log('\n── FIX 4: T14 — ssr: false routes bypass runner ──\n');
{
    ...
    const noHydrationScript = !response.hasHydrationState && !response.body.includes('__NUXCO_STATE__');
