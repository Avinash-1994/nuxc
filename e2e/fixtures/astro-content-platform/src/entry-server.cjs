 ...
 *
 * Real server module for Nuxc Phase 2.6 — Astro Islands + Content Collections.
 *
 * Implements:
...
function renderPage(url, opts) {
  ...
  var title = isHome ? 'Home | Nuxc Astro Platform'
    : isBlogIndex ? 'Blog | Nuxc Astro Platform'
    : isBlogPost && post ? post.data.title + ' | Blog'
    : isDocsIndex ? 'Docs | Nuxc Astro Platform'
    : isAbout ? 'About | Nuxc Astro Platform'
    : 'Nuxc Astro Platform';
  ...
  var html = [
    ...
    '  <title>' + title + '</title>',
    '  <meta name="description" content="Nuxc Astro Platform — Phase 2.6 SSG" />',
    ...
    '<footer><p>Powered by Nuxc + Astro · Phase 2.6</p></footer>',
    '</body>',
    '</html>',
  ].join('\n');
...
function _buildPageContent(url, isHome, isBlogIndex, isBlogPost, isDocsIndex, isAbout, blogPosts, docPages, post) {
  ...
  if (isAbout) {
    return [
      '<h1>About Nuxc</h1>',
      '<p>Nuxc is a next-generation build system for modern web frameworks.</p>',
      '<ul>',
...
function emitBuildArtifacts(appRoot, outDir) {
  ...
  allIslands.forEach(function(isl, chunkName) {
    var code = [
      ...
      '// Generated by Nuxc Astro Adapter (Phase 2.6)',
      ...
      '  console.log("[Nuxc Astro] Hydrating ' + isl.component + ' with directive ' + isl.directive + '", props);',
