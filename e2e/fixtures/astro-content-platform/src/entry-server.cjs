 ...
 *
 * Real server module for Sparx Phase 2.6 — Astro Islands + Content Collections.
 *
 * Implements:
...
function renderPage(url, opts) {
  ...
  var title = isHome ? 'Home | Sparx Astro Platform'
    : isBlogIndex ? 'Blog | Sparx Astro Platform'
    : isBlogPost && post ? post.data.title + ' | Blog'
    : isDocsIndex ? 'Docs | Sparx Astro Platform'
    : isAbout ? 'About | Sparx Astro Platform'
    : 'Sparx Astro Platform';
  ...
  var html = [
    ...
    '  <title>' + title + '</title>',
    '  <meta name="description" content="Sparx Astro Platform — Phase 2.6 SSG" />',
    ...
    '<footer><p>Powered by Sparx + Astro · Phase 2.6</p></footer>',
    '</body>',
    '</html>',
  ].join('\n');
...
function _buildPageContent(url, isHome, isBlogIndex, isBlogPost, isDocsIndex, isAbout, blogPosts, docPages, post) {
  ...
  if (isAbout) {
    return [
      '<h1>About Sparx</h1>',
      '<p>Sparx is a next-generation build system for modern web frameworks.</p>',
      '<ul>',
...
function emitBuildArtifacts(appRoot, outDir) {
  ...
  allIslands.forEach(function(isl, chunkName) {
    var code = [
      ...
      '// Generated by Sparx Astro Adapter (Phase 2.6)',
      ...
      '  console.log("[Sparx Astro] Hydrating ' + isl.component + ' with directive ' + isl.directive + '", props);',
