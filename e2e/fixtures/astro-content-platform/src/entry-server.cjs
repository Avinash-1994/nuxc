 ...
 *
 * Real server module for Zeptr Phase 2.6 — Astro Islands + Content Collections.
 *
 * Implements:
...
function renderPage(url, opts) {
  ...
  var title = isHome ? 'Home | Zeptr Astro Platform'
    : isBlogIndex ? 'Blog | Zeptr Astro Platform'
    : isBlogPost && post ? post.data.title + ' | Blog'
    : isDocsIndex ? 'Docs | Zeptr Astro Platform'
    : isAbout ? 'About | Zeptr Astro Platform'
    : 'Zeptr Astro Platform';
  ...
  var html = [
    ...
    '  <title>' + title + '</title>',
    '  <meta name="description" content="Zeptr Astro Platform — Phase 2.6 SSG" />',
    ...
    '<footer><p>Powered by Zeptr + Astro · Phase 2.6</p></footer>',
    '</body>',
    '</html>',
  ].join('\n');
...
function _buildPageContent(url, isHome, isBlogIndex, isBlogPost, isDocsIndex, isAbout, blogPosts, docPages, post) {
  ...
  if (isAbout) {
    return [
      '<h1>About Zeptr</h1>',
      '<p>Zeptr is a next-generation build system for modern web frameworks.</p>',
      '<ul>',
...
function emitBuildArtifacts(appRoot, outDir) {
  ...
  allIslands.forEach(function(isl, chunkName) {
    var code = [
      ...
      '// Generated by Zeptr Astro Adapter (Phase 2.6)',
      ...
      '  console.log("[Zeptr Astro] Hydrating ' + isl.component + ' with directive ' + isl.directive + '", props);',
