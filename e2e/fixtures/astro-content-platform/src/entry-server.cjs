 ...
 *
 * Real server module for Nuce Phase 2.6 — Astro Islands + Content Collections.
 *
 * Implements:
...
function renderPage(url, opts) {
  ...
  var title = isHome ? 'Home | Nuce Astro Platform'
    : isBlogIndex ? 'Blog | Nuce Astro Platform'
    : isBlogPost && post ? post.data.title + ' | Blog'
    : isDocsIndex ? 'Docs | Nuce Astro Platform'
    : isAbout ? 'About | Nuce Astro Platform'
    : 'Nuce Astro Platform';
  ...
  var html = [
    ...
    '  <title>' + title + '</title>',
    '  <meta name="description" content="Nuce Astro Platform — Phase 2.6 SSG" />',
    ...
    '<footer><p>Powered by Nuce + Astro · Phase 2.6</p></footer>',
    '</body>',
    '</html>',
  ].join('\n');
...
function _buildPageContent(url, isHome, isBlogIndex, isBlogPost, isDocsIndex, isAbout, blogPosts, docPages, post) {
  ...
  if (isAbout) {
    return [
      '<h1>About Nuce</h1>',
      '<p>Nuce is a next-generation build system for modern web frameworks.</p>',
      '<ul>',
...
function emitBuildArtifacts(appRoot, outDir) {
  ...
  allIslands.forEach(function(isl, chunkName) {
    var code = [
      ...
      '// Generated by Nuce Astro Adapter (Phase 2.6)',
      ...
      '  console.log("[Nuce Astro] Hydrating ' + isl.component + ' with directive ' + isl.directive + '", props);',
