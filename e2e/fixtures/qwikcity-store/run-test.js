...
await (async function testServerActions() {
  const cartPayload = { productId: 'nuxc-pro-kit', quantity: 2 };
...
await (async function testColdStart() {
  ...
  (coldMs < gate ? printPass : printFail)(
    ...
    `Environment: ${env}`,
    `[nuxc] adapter: qwik-city in output: yes`,
...
await (async function testHmr() {
  ...
  const changeDesc = 'appended comment // nuxc-hmr-ts (triggers module invalidation)';
...
log('');
log('┌─────────────────────────────────────────────┐');
log('│ NUXC — PHASE 2.5 QWIK CITY COMPLETE       │');
log('│                                             │');
...
log(`│ [nuxc] adapter: qwik-city confirmed        │`);
