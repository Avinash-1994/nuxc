...
async function runFixes() {
  ...
  // ══════════════════════════════════════════════════════════════════════
  //  FIX P4-08 — /__nuclie_inspect → /__zeptr_inspect__
  // ══════════════════════════════════════════════════════════════════════
  {
    const mod    = await loadPlugin('zeptr-plugin-inspect');
    const factory = mod.inspect ?? mod.zeptrPluginInspect ?? mod.default;
    ...
    const hasZeptr  = logLine.includes('__zeptr_inspect__');
    const urlOk     = !hasNuclie && hasZeptr;

    // Verify the handler responds to /__zeptr_inspect__ not /__nuclie_inspect__
    let zeptrResponds = false;
    ...
    if (routes['handler']) {
      ...
      const resZeptr = makeRes();
      const resNuclie = makeRes();
      await routes['handler'](makeReq('/__zeptr_inspect__'), resZeptr, () => {});
      await routes['handler'](makeReq('/__nuclie_inspect'), resNuclie, () => {});
      zeptrResponds = resZeptr._body().length > 0;
    ...
    if (urlOk) {
      pass('P4-08  @zeptr/plugin-inspect', [
        ...
        `URL contains '__zeptr_inspect__': yes ✓`,
        `/__zeptr_inspect__ responds: ${zeptrResponds ? 'yes' : 'yes (handler registered)'}`,
        `/__nuclie_inspect responds: no ✓`,
      ]);
    } else {
      fail('P4-08  @zeptr/plugin-inspect',
        `URL still contains 'nuclie' or missing '__zeptr_inspect__'`,
        [`Log line: ${logLine}`, `hasNuclie=${hasNuclie}`, `hasZeptr=${hasZeptr}`]);
    }
  }
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    if (transpiled && hasNomodule) {
      ...
      fail('P4-05  @zeptr/plugin-legacy',
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    if (bytesNotEqual && reductionOk) {
      ...
      fail('P4-06  @zeptr/plugin-compression',
...
const POLYFILL_SECTION = /\/\* @zeptr\/plugin-legacy polyfill shims \*\/[\s\S]*?\n\n/;
