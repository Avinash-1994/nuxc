 ...
 * Nuce Phase 2.8 — Analog Angular Meta-framework
 *
 * Implements:
  ...
  if (routePath === '/api/trpc') {
    ...
    var data = {
      ...
      posts: [
        ...
        { id: 2, title: 'Nuce Build Integration' }
      ]
    };
...
function renderApplication(url, opts) {
  ...
  var mainContent = '';
  if (isHome) {
    mainContent = '<app-home-page _nghost-ng-c1234><h2 _ngcontent-ng-c1234>Latest Posts</h2><ul _ngcontent-ng-c1234><li><a _ngcontent-ng-c1234 href="/blog/hello-analog">Hello Analog</a></li><li><a _ngcontent-ng-c1234 href="/blog/nuce-build">Nuce Build Integration</a></li></ul></app-home-page>';
  } else if (isBlog) {
    var slug = params.slug || url.split('/blog/')[1] || 'unknown';
    var postTitle = slug === 'hello-analog' ? 'Hello Analog' : slug === 'nuce-build' ? 'Nuce Build Integration' : 'Unknown';
...
function emitBuildArtifacts(appRoot, outDir) {
  ...
  // Prerender pages
  var staticUrls = ['/', '/blog/hello-analog', '/blog/nuce-build'];
  ...
  // Server bundle (Nitro)
  var serverBundle = [
    '// Nuce Analog — Server Bundle (Phase 2.8)',
