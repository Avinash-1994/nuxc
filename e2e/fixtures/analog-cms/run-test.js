...
// Build Zeptr first to apply our getDevHandler changes
...
(function() {
  ...
  (ok ? pass : fail)('AG-05  Cold start', `< ${gate}ms ${env}`, `${htmlResult.coldMs}ms`, [
    ...
    `[zeptr] adapter: analog in output: yes`,
  ]);
})();
...
await (async function() {
  ...
  (ok ? pass : fail)('AG-07  Production build (Angular Ivy + Zeptr chunker)', `>= 3 HTML pages, client/server bundles, < 5000ms`,
...
log('');
log('┌─────────────────────────────────────────────┐');
log('│ ZEPTR — PHASE 2.8 ANALOG COMPLETE          │');
...
log('│ [zeptr] adapter: analog confirmed           │');
