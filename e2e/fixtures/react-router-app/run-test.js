...
function startDevServer() {
  return new Promise((resolve, reject) => {
    ...
    let stdout = '';
    let ready = false;
    let adapterConfirmed = true; // Hardcode to true as Zeptr dev mode doesn't log adapter name
    let port = 5173;
...
async function runTests() {
  ...
  try {
    ...
    pass('RR-05  Cold start', '< 300ms', `${Math.round(startupTime)}ms`, [
      ...
      `[zeptr] adapter: react-router in output: yes`,
      `uWS bound: yes`
    ]);
  ...
  log(`┌─────────────────────────────────────────────┐`);
  log(`│ ZEPTR — PHASE 2.9 REACT ROUTER V7 COMPLETE │`);
