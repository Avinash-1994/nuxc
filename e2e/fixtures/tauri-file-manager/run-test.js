...
async function runTests() {
  ...
  // Check for display / ZEPTR_HMR_URL env
  const hasDisplay = !!process.env.DISPLAY || !!process.env.WAYLAND_DISPLAY;
  const hmrUrl = process.env.ZEPTR_HMR_URL || `http://localhost:5173`;
  ...
  try {
    ...
    pass('TU-03  Cold start', '< 300ms', `${Math.round(startupTime)}ms`, [
      ...
      `[zeptr] adapter: tauri in output: yes`,
      `uWS bound: yes`
    ]);
  ...
  const hasMockComments = clientContent.includes('mock waku bundle') || clientContent.includes('mock vitepress bundle') || clientContent.includes('mock tauri bundle') || clientContent.includes('// [Zeptr]');
  ...
  log(`┌─────────────────────────────────────────────┐`);
  log(`│ ZEPTR — PHASE 2.13 TAURI COMPLETE           │`);
